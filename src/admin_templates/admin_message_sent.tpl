					...
					<tr>
						<td align="center">
							<form name="listFrom" id="listForm" action="" method="post">
                            	...
                            	<input type="hidden" name="sent_msg" value="sent_messages" />
...
<!--                                        	<option value="message_from" selected="selected" >Message From</option>-->
											<option value="message_sent_dt"  selected="selected" >Date</option>
                                        	<option value="message_subject" {if $search == "message_subject"} selected="selected"{/if} >Subject</option>
...
<!--                                        	<option value="monthly" {if $search == "monthly"} selected="selected"{/if} >Monthly</option>-->
										...
										<tr class="header_bgcolor" height="26">
											...
											<td align="center" class="headertext" width="25%">To</td>
										...
										{section name=counter loop=$messageRows}
											<tr class="{cycle values="odd_tr,even_tr"}">
                                                ...
                                                <td align="center" class="smalltext">{$messageRows[counter].message_to_username}</td>
                                                <td align="center" class="smalltext"><a href="{$adminActualPath}/admin_messages.php?mode=read&from=sent&message_id={$messageRows[counter].message_id}&encoded_string={$encoded_string}">{$messageRows[counter].message_subject}</a></td>
                                                <td align="center" class="smalltext">{$messageRows[counter].send_date}</td>
												<td align="center" class="bold_text">
													<a href="#" class="view_link" onclick="javascript: deleteConfirmRecord('{$adminActualPath}/admin_messages.php?mode=delete_sent_message&message_id={$messageRows[counter].message_id}&encoded_string={$encoded_string}', 'Message'); return false;"><img src="{$smarty.const.CLOUD_STATIC_ADMIN}delete_image.png" align="absmiddle" alt="Delete Message" title="Delete Message" border="0" class="changeStatus" /></a>
												</td>
											</tr>
										{/section}
										<tr class="header_bgcolor" height="26">
