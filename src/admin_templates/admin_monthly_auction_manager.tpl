										...
										{section name=counter loop=$auctionRows}
											<tr id="tr_{$auctionRows[counter].auction_id}" class="{cycle values="odd_tr,even_tr"}">
                                                ...
                                                {*if $auctionRows[counter].auction_is_approved == '0'*}
                                                    ...
                                                    <a href="#" class="view_link" onclick="javascript: deleteConfirmRecord('{$adminActualPath}/admin_auction_manager.php?mode=delete_auction&auction_id={$auctionRows[counter].auction_id}&auction_type=3&encoded_string={$encoded_string}', 'auction'); return false;"><img src="{$smarty.const.CLOUD_STATIC_ADMIN}delete_image.png" align="absmiddle" alt="Delete Auction" title="Delete Auction" border="0" class="changeStatus" /></a>
                                                {*/if*}   
