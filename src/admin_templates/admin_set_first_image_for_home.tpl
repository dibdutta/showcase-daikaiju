						...
						<table align="center" width="80%" border="0" cellspacing="1" cellpadding="2" class="header_bordercolor" >
							<tr>
							   ...
							   <td>
								...
								&nbsp;<input type="button"  value="View All" class="button" onclick="window.location.href='admin_set_first_image_for_home.php?mode=fixed&search={$smarty.request.search}'">
								</td>
							</tr>
							</table>
						</form>	
										...
										{section name=counter loop=$auctionRows}
											<tr id="tr_{$auctionRows[counter].auction_id}" class="{cycle values="odd_tr,even_tr"}">
												...
												<td align="center" class="bold_text">
													...
													{if  $search == 'selling'}
                       									<input type="radio" name="admin_track_slider" value="{$auctionRows[counter].auction_id}" title="Select First Item for Home Page" {if $auctionRows[counter].slider_first_position_status==1} checked="checked";{/if} onclick="changeSeliderPositionStatus({$auctionRows[counter].auction_id},'2');" >
                      								{/if}
													{if $search == 'sold'}
													<input type="radio" name="admin_track_slider" value="{$auctionRows[counter].auction_id}" title="Select First Item for Home Page" {if $auctionRows[counter].slider_first_position_status==1} checked="checked";{/if} onclick="changeSeliderPositionStatus({$auctionRows[counter].auction_id},'1');" >
													{/if}
												</td>
											</tr>
										{/section}
