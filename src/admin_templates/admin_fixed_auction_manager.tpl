							...
							<tr>
								<td class="bold_text" align="center" colspan="2" >
										...
										<select name="search" class="look" onchange="javascript: this.form.submit();">
                                        	...
                                        	<option value="all" {if $search == "all"}selected="selected"{/if} >All</option>
											...
											<option value="seller_pending" {if $search == "seller_pending"} selected="selected"{/if}>Sale Pending</option>
										</select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								...
								<table align="center" width="80%" border="0" cellspacing="1" cellpadding="2" class="header_bordercolor" >
									<tbody>
										<tr class="header_bgcolor" height="26">
                                            ...
                                            {if $smarty.request.search == '' || $smarty.request.search == 'selling' || $smarty.request.search == 'pending'}<td align="center" class="headertext" width="8%">Status</td>{/if}
										...
										{section name=counter loop=$auctionRows}
											<tr id="tr_{$auctionRows[counter].auction_id}" class="{cycle values="odd_tr,even_tr"}">
                                                ...
                                                <td align="center" class="smalltext">{if $auctionRows[counter].auction_reserve_offer_price > 0}Will consider offers{else}--{/if}</td>
                                                {if $smarty.request.search == '' || $smarty.request.search == 'selling' || $smarty.request.search == 'pending'}
                                                <td id="td_{$auctionRows[counter].auction_id}" align="center" class="smalltext">
													...
													{elseif $auctionRows[counter].auction_is_approved == 1}
														...
														<img src="{$smarty.const.CLOUD_STATIC_ADMIN}icon_inactive.gif" align="absmiddle" alt="Disapprove" border="0" onclick="javascript: approveAuction({$auctionRows[counter].auction_id}, 2, '{$smarty.request.search}', 'fixed');" title="Disapprove" class="changeStatus" />
														{/if}
                                                    {else}
                                                    	Disapproved
													{/if}
                                                ...
                                                {*if $auctionRows[counter].auction_is_approved == 0*}
                                                	<a href="{$adminActualPath}/admin_auction_manager.php?mode=edit_fixed&auction_id={$auctionRows[counter].auction_id}&encoded_string={$encoded_string}" class="view_link"><img src="{$smarty.const.CLOUD_STATIC_ADMIN}icon_edit.gif" align="absmiddle" alt="Update" title="Update" border="0" class="changeStatus" /></a>
                                                    <a href="#" class="view_link" onclick="javascript: deleteConfirmRecord('{$adminActualPath}/admin_auction_manager.php?mode=delete_auction&auction_id={$auctionRows[counter].auction_id}&auction_type=1&encoded_string={$encoded_string}', 'auction','{$auctionRows[counter].auction_id}'); return false;"><img src="{$smarty.const.CLOUD_STATIC_ADMIN}delete_image.png" align="absmiddle" alt="Delete Auction" title="Delete Auction" border="0" class="changeStatus" /></a> 
                                                {*/if*}
                                                 <a href="{$adminActualPath}/admin_auction_manager.php?mode=view_fixed&auction_id={$auctionRows[counter].auction_id}&encoded_string={$encoded_string}" class="view_link"><img src="{$smarty.const.CLOUD_STATIC_ADMIN}icon_open.gif" align="absmiddle" alt="Details" title="Details" border="0" class="changeStatus" /></a> 
                                                    <!--&nbsp;|&nbsp;<a href="#" class="view_link"><img src="{$smarty.const.CLOUD_STATIC_ADMIN}delete_image.png" align="absmiddle" alt="Delete" title="Delete" border="0" class="changeStatus" /></a>&nbsp;&nbsp;-->
                                             		{if $search == 'selling' || $search == 'sold' || $auctionRows[counter].auction_is_approved == '1'}<a href="{$adminActualPath}/admin_auction_manager.php?mode=view_details_offer&auction_id={$auctionRows[counter].auction_id}&encoded_string={$encoded_string}" class="view_link"><img src="{$smarty.const.CLOUD_STATIC_ADMIN}icon_view.gif" align="absmiddle" alt="View Bid Details" title="View Bid Details" border="0" class="changeStatus" /></a>{/if}
													{if $search == 'sold'}
													...
													&nbsp;&nbsp;
													{if $search == 'sold'}
													{if $auctionRows[counter].is_shipped=='1'}
														<a href="javascript:void(0)"><img src="{$smarty.const.CLOUD_STATIC_ADMIN}shipped.png" alt="Combined" title="Shipped on {$auctionRows[counter].shipped_date|date_format:"%m/%d/%Y"}" class="changeStatus" > </a>
													{else}
														<a href="javascript:void(0)"><img src="{$smarty.const.CLOUD_STATIC_ADMIN}not_shipped.png" alt="Combined" title="Not Shipped" class="changeStatus_{$auctionRows[counter].invoice_id}" onclick="mark_shipped_buyer_invoice('{$auctionRows[counter].invoice_id}')"></a>
													{/if}
													{/if}
													...
													{/if}
													{if $search == 'selling'}
													<a id="various_{$smarty.section.counter.index}" href="{$actualPath}/admin_myselling.php?mode=move_to_weekly&id={$auctionRows[counter].auction_id}"><input type="button" id="{$smarty.section.counter.index}" onclick="fancy_images(this.id)" class="track-btn" value="Move to weekly" /></a>
													{/if}
												</td>
											</tr>
										{/section}
