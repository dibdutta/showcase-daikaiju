										...
										{section name=counter loop=$auctionRows}
											<tr id="tr_{$auctionRows[counter].auction_id}" class="{cycle values="odd_tr,even_tr"}">
												...
												<td align="center" class="bold_text">
													...
													{if  $search == 'selling'}
                       									<input type="radio" name="admin_track_slider" value="{$auctionRows[counter].auction_id}" title="Select First Item for Home Page" {if $auctionRows[counter].slider_first_position_status==1} checked="checked";{/if} onclick="changeSeliderPositionStatus({$auctionRows[counter].auction_id},'3');" >
                      								{/if}
