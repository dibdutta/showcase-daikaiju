		...
		if($bid_amount >= $next_highest_bid_amnt){
			if($counter > 0){
				...
				#### new added if two highest proxy bid  is of same value ######
						...
						$sql_bid_insert="Insert into tbl_bid (bid_fk_user_id,bid_fk_auction_id,bid_amount,is_proxy,post_date,post_ip,is_snipe) values ('".$user_id."','".$auction_id."','".$bid_amount."','0','".date('Y-m-d H:i:s')."','".$_SERVER['REMOTE_ADDR']."','1')";
				 	...
				 	if($highest_bid_amnt <= $second_highest_val)
				 	{
					 	...
					 	if($totBid < 1){
					 		if($highest_proxy_amnt != $bid_amount){
								...
								"bid_is_won" => 0, "post_date" => date("Y-m-d H:i:s"), "post_ip" => $_SERVER['HTTP_HOST'],"is_snipe"=>1);
								...
								//$out_bid_key='outbid';
					 			...
					 			$sql_bid_insert="Insert into tbl_bid (bid_fk_user_id,bid_fk_auction_id,bid_amount,is_proxy,post_date,post_ip,is_snipe) values ('".$highest_user."','".$auction_id."','".$bid_amount_for_new_proxy."','1','".date('Y-m-d H:i:s')."','".$_SERVER['REMOTE_ADDR']."','1')";
								$sql_bid_insert_res=mysqli_query($GLOBALS['db_connect'],$sql_bid_insert);
								$bid=1;
                                $latest_bid_count= $latest_bid_count+1;
						...
						if($counter_tot_high > 1){
				 		...
				 		if($totBid<1){
			 				$sql_bid_insert="Insert into tbl_bid (bid_fk_user_id,bid_fk_auction_id,bid_amount,is_proxy,post_date,post_ip,is_snipe) values ('".$user_id."','".$auction_id."','".$bid_amount_for_winner."','1','".date('Y-m-d H:i:s')."','".$_SERVER['REMOTE_ADDR']."','1')";
							$sql_bid_insert_res=mysqli_query($GLOBALS['db_connect'],$sql_bid_insert);
                             $latest_bid_count= $latest_bid_count+1;
				 		...
				 		if($second_highest_val >= $highest_bid_amnt){
							...
							$tot_counter_row_highest_new_counter=$tot_counter_row_highest_new['0'];
			 	 			if($tot_counter_row_highest_new_counter==0){
								$sql_bid_insert_new="Insert into tbl_bid (bid_fk_user_id,bid_fk_auction_id,bid_amount,is_proxy,post_date,post_ip,is_snipe) values ('".$second_highest_user."','".$auction_id."','".$second_highest_val."','0','".date('Y-m-d H:i:s')."','".$_SERVER['REMOTE_ADDR']."','1')";
  	...
  	if($bid!='try_later'){
			...
			if($counter_tot_second==0 && $second_highest_user!=0){
				...
				$tot_counter_row_highest_new_counter=$tot_counter_row_highest_new['0'];
 	 			...
 	 			if($tot_counter_row_highest_new_counter==0){
					$sql_bid_insert_new="Insert into tbl_bid (bid_fk_user_id,bid_fk_auction_id,bid_amount,is_proxy,post_date,post_ip,is_snipe) values ('".$highest_user."','".$auction_id."','".$highest_proxy_amnt."','1','".date('Y-m-d H:i:s')."','".$_SERVER['REMOTE_ADDR']."','1')";
					$sql_bid_insert_res_new=mysqli_query($GLOBALS['db_connect'],$sql_bid_insert_new);
                    $latest_bid_count= $latest_bid_count+1;

                    $sql_bid_insert_new_new="Insert into tbl_bid (bid_fk_user_id,bid_fk_auction_id,bid_amount,is_proxy,post_date,post_ip,is_snipe) values ('".$user_id."','".$auction_id."','".$bid_amount."','0','".date('Y-m-d H:i:s')."','".$_SERVER['REMOTE_ADDR']."','1')";
