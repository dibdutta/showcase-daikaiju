...
if($_REQUEST['mode'] == "edit_user"){
	edit_user();
}elseif($_REQUEST['mode'] == "add_user"){
	add_user();
}elseif($_REQUEST['mode'] == "save_user"){
...
}elseif($_REQUEST['mode'] == "update_user"){
	$chk = validateForm();
	if($chk == true){
		update_user();
	}else{
	...
	delete_user();
}elseif($_REQUEST['mode'] == "delete_all_user"){
	delete_all_user();
...
function validateForm()
{
	...
	if($_REQUEST['email'] == ""){
		...
		if($obj->checkUnique(EMAIL,$_REQUEST['email'], $_REQUEST['user_id'])){
...
function update_user()
{
	...
	if($obj->updateUser()){	
		...
		$obj->updateData(CARD_DETAIL, $cardData, array('user_id' => $_REQUEST['user_id']), true);
...
function add_user(){
	...
	foreach ($_REQUEST as $key => $value ) {
		eval('$smarty->assign("'.$key.'", $_REQUEST["'.$key.'"]);'); 
	}
