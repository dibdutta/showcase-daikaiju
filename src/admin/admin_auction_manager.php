...
}elseif($mode == "edit_weekly"){
	edit_weekly();
}elseif($mode == "update_weekly"){
...
}elseif($mode == "edit_monthly"){
	edit_monthly();
}elseif($mode == "update_monthly"){
...
}elseif($mode == "manage_invoice_seller"){
...
elseif($mode == "manage_invoice"){
	...
	monthlyAuction();
}elseif($mode == "phone_order"){
	phone_order();
}elseif($mode == "view_details"){
	view_details();
}elseif($mode == "view_details_offer"){
	view_details_offer();
}elseif($mode == "view_monthly"){
	view_monthly();
}elseif($mode == "view_fixed"){
	view_fixed();
}elseif($mode == "view_weekly"){
	view_weekly();
}elseif($mode == "update_invoice_charge"){
	update_invoice_charge();
}elseif($mode == "update_invoice_discount"){
	update_invoice_discount();
}elseif($mode == "delete_invoice_charge"){
	delete_invoice_charge();
}elseif($mode == "delete_invoice_discount"){
	delete_invoice_discount();
}elseif($mode == "approve_invoice"){
	approve_invoice();
}elseif($mode == "cancel_invoice"){
	cancel_invoice();
}elseif($mode == "notify_buyer"){
    notify_buyer();
}elseif($mode == "reopen_fixed"){
	reopen_fixed();
}elseif($mode == "reopen_fixed_auction"){
	reopen_fixed_auction();
}elseif($mode == "reopen_weekly"){
	reopen_weekly();
}elseif($mode == "reopen_weekly_auction"){
	reopen_weekly_auction();
}elseif($mode == "reopen_monthly"){
	reopen_monthly();
}elseif($mode == "reopen_monthly_auction"){
	reopen_monthly_auction();
}elseif($mode == "manage_invoice_seller_print"){
	manage_invoice_seller_print();
}elseif($mode == "delete_auction"){
	delete_auction();
}elseif($mode == "create_fixed"){
	create_fixed();
}elseif($mode == "save_fixed_auction"){
...
}elseif($mode == "bulkupload_pending"){
    bulkupload_pending();
}elseif($mode == "download_bulk"){
    download_bulk();
}elseif($mode == "delete_bulk"){
    delete_bulk();
}elseif($mode == "track_first_sold_for_home"){
    track_first_sold_for_home();
}elseif($mode == "update_slider"){
 updateSlider();
}else if($mode == "clearOffer"){
 clearOffer();
}else if($mode == "mark_as_paid_invoice"){
    mark_as_paid_invoice();
}else if($mode == "stills"){
    stills();
}else if($mode == "create_stills"){
    create_stills();
}elseif($mode == "save_stills"){
...
}elseif($mode == "edit_stills"){
	edit_stills();
}elseif($mode == "update_stills"){
	$chk = validateFixedForm();
	if($chk == true){
		update_stills();
	}else{
...
}elseif($mode == "weekly_relist"){
    weekly_relist();
}elseif($mode == "relist_weekly_to_weekly"){
    relist_weekly_to_weekly();
}elseif($mode == "shipped_item_list"){
    shipped_item_list();
}else if($mode == "stills_auction"){
    stills_auction();
}else if($mode == "create_stills_auction"){
    create_stills_auction();
}elseif($mode == "save_stills_auction"){
...
}elseif($mode == "edit_stills_auction"){
	edit_stills_auction();
}elseif($mode == "update_stills_auction"){
...
}elseif($mode == "view_stills"){
	view_stills();
}elseif($mode == "reopen_stills"){
    reopen_stills();
}elseif($mode == "reopen_stills_auction"){
    reopen_stills_auction();
}elseif($mode == "set_as_big_slider"){
    set_as_big_slider();
}elseif($mode == "remove_as_big_slider"){
    remove_as_big_slider();
}elseif($mode == "add_note"){
    add_note();
}elseif($mode == "invoice_bulk"){
    manage_invoice_seller_print_bulk();
}else{
	fixedPriceSale();
}
...
function weeklyAuction() {
	...
	if($start_date_req != '' && $end_date_req != '' && $start_date_req > $end_date_req){
		$_SESSION['adminErr'] = "End Date must be greater than Start Date.";
		header("location: ".PHP_SELF."?mode=weekly&search=".$search);
	}else{
	$auctionObj = new Auction();
	$auctionObj->orderType = 'DESC';
	if($start_date_req!='')
	...
	if($search!='sold'){
			$total = $auctionObj->countWeeklyAuctionByStatus($search,'',$search_fixed_poster,$start_date,$end_date);
		}else{
			$total = $auctionObj->soldAuctionCOUNTWEEKLY($search,'',$search_fixed_poster,$start_date,$end_date);
		}

	if($total>0){
	if($search!='sold'){
		$auctionRows = $auctionObj->fetchWeeklyAuctionByStatus($search,'',$sort_type,$search_fixed_poster,$start_date,$end_date);
		}else{
			$auctionObj->orderBy='invoice_generated_on';
			$auctionRows=$auctionObj->soldAuctionWEEKLY($search,'',$sort_type,$search_fixed_poster,$start_date,$end_date);
		}
		$total_now=count($auctionRows ?? []);
    	for($i=0;$i<$total_now;$i++)
    	{
...
if(!$_POST)
{
	...
	for($i=0;$i<count($posterImageRows ?? []);$i++){
		$existingImages = ($existingImages ?? '') . $posterImageRows[$i]['poster_image'].",";
	}
	...
	$smarty->assign('poster_desc', stripslashes($auctionRow[0]['poster_desc']));
...
if(!$_POST)
{
	...
	$auctionRow = $auctionObj->fetchEditAuctionDetail($_REQUEST['auction_id'],$chkLive);
	if($chkLive>0){
		$posterImageRows =  $auctionObj->selectData('tbl_poster_images_live', array('*'), array("fk_poster_id" => $auctionRow[0]['fk_poster_id']));
	}else{
		$posterImageRows =  $auctionObj->selectData(TBL_POSTER_IMAGES, array('*'), array("fk_poster_id" => $auctionRow[0]['fk_poster_id']));
	}
	
    for($i=0;$i<count($posterImageRows ?? []);$i++){
        if ($posterImageRows[$i]['is_cloud']!='1'){
            $posterImageRows[$i]['image_path']=CLOUD_POSTER_THUMB.$posterImageRows[$i]['poster_thumb'];
        }else{
            $posterImageRows[$i]['image_path']=CLOUD_POSTER_THUMB.$posterImageRows[$i]['poster_thumb'];
        }
	...
	if($chkLive>0){
		$posterCategoryRows =  $auctionObj->selectData('tbl_poster_to_category_live', array('fk_cat_id'), array("fk_poster_id" => $auctionRow[0]['fk_poster_id']));
	}else{
		$posterCategoryRows =  $auctionObj->selectData(TBL_POSTER_TO_CATEGORY, array('fk_cat_id'), array("fk_poster_id" => $auctionRow[0]['fk_poster_id']));
	}
	...
	$subcatObj = new Subcategory();
	$smarty->assign('subcatJson', json_encode($subcatObj->fetchAllGrouped()));
	$smarty->assign('selected_shop_cat_id', $shopCatObj->getPosterShopCatId($auctionRow[0]['fk_poster_id'] ?? 0, true));
...
function view_weekly()
{
	...
	if($chkLive>0){
		...
		$posterImageRows =  $auctionObj->selectData(TBL_POSTER_IMAGES, array('*'), array("fk_poster_id" => $auctionRow[0]['fk_poster_id']));
		for($i=0;$i<count($posterImageRows ?? []);$i++){
			if ($posterImageRows[$i]['poster_thumb']!='1'){
				$posterImageRows[$i]['image_path']=CLOUD_POSTER_THUMB.$posterImageRows[$i]['poster_thumb'];
			}else{
				$posterImageRows[$i]['image_path']=CLOUD_POSTER_THUMB.$posterImageRows[$i]['poster_thumb'];
			}
...
function update_weekly(){
	...
	$obj->updateData('tbl_auction_live', $auctionData, array("auction_id" => $auction_id), true);	
	...
	$obj->updateData('tbl_poster_images_live', array("is_default" => 0), array("fk_poster_id" => $poster_id), true);
	$obj->updateData('tbl_poster_images_live', array("is_default" => 1), array("original_filename" => $is_default,"fk_poster_id"=>$poster_id), true);
	$obj->updateData('tbl_poster_images_live', array("is_default" => 1), array("poster_thumb" => $is_default,"fk_poster_id"=>$poster_id), true);

	//if($chk){
		$_SESSION['adminErr'] = "Auction has been updated successfully.";
		header("location: ".DOMAIN_PATH."".easy_decrypt($_REQUEST['encoded_string'] ?? '')."#tr_".$auction_id);
		exit;
	/*}else{
		$_SESSION['adminErr'] = "No category has not been created successfully.";
...
//	print_r($auctionArr);
	...
	if($auctionArr[0]['auction_is_sold']=='1'){
		//$objBid = new Bid();
		$auctionArr=$objBid->fetchBidsByIdArchiveS($auctionArr);
		//echo "<pre>".print_r($auctionArr)."</pre>";
		$objBid->fetch_BidCount_MaxBidArchive($auctionArr);
		$countBid=count($auctionArr[0]['bids']);
...
function manage_invoice()
{
	...
	usort($invoiceData['auction_details'], 'compareByName');
	...
	if($chk_item_type==1 || $chk_item_type==4){
		  ...
		  foreach($invoiceData['auction_details'] as $k=>$v) {
			$sort['poster_title'][$k] = $v['poster_title'];
			$sort['seller_username'][$k] = $v['seller_username'];
		  }
		  # sort by event_type desc and then title asc
		  array_multisort($sort['seller_username'], SORT_DESC, $sort['poster_title'], SORT_ASC,$invoiceData['auction_details']);
	  }
	if($chk_item_type==1 || $chk_item_type==4){
...
function manage_invoice_seller_print(){
	...
	$invoiceData[0]['shipping_address'] = preg_replace_callback('!s:(\d+):"(.*?)";!s', function($m) { return 's:'.strlen($m[2]).':"'.$m[2].'";'; }, $invoiceData[0]['shipping_address']);
	$invoiceData['shipping_address'] = unserialize($invoiceData[0]['shipping_address']);
	$invoiceData[0]['billing_address'] = preg_replace_callback('!s:(\d+):"(.*?)";!s', function($m) { return 's:'.strlen($m[2]).':"'.$m[2].'";'; }, $invoiceData[0]['billing_address']);
	...
	$sql = "SELECT a.auction_actual_end_datetime from tbl_invoice_to_auction ita,tbl_auction a
...
function delete_invoice_charge(){
 ...
 function update_invoice_discount(){
	...
	$auction[0]['discounts']=preg_replace_callback('!s:(\d+):"(.*?)";!s', function($m) { return 's:'.strlen($m[2]).':"'.$m[2].'";'; }, $auction[0]['discounts']);
     ...
     if(isset($choose_fixed_weekly) && $choose_fixed_weekly!=''){
         ...
         if($choose_fixed_weekly=='fixed'){
            if(isset($fixed_asked_price) && $fixed_asked_price!=''){
				if(!isset($is_consider) || $is_consider==0){
					...
					if($poster_size!='34'){
					$auctionData = array("fk_auction_type_id"=>'1',"fk_poster_id"=>$poster_id,"auction_is_approved"=>'1',"is_reopened"=>'1',"auction_is_sold"=>'0',"fk_auction_week_id" => '', "auction_asked_price" => $fixed_asked_price, "auction_buynow_price" => '',"auction_reserve_offer_price"=>$offer_price,"auction_start_date" => '', "auction_end_date" => '',
						"auction_actual_start_datetime" => '', "auction_actual_end_datetime" => '',"post_date"=>date('Y-m-d H:i:s'));
						}else{
					$auctionData = array("fk_auction_type_id"=>'4',"fk_poster_id"=>$poster_id,"auction_is_approved"=>'1',"is_reopened"=>'1',"auction_is_sold"=>'0',"fk_auction_week_id" => '', "auction_asked_price" => $fixed_asked_price, "auction_buynow_price" => '',"auction_reserve_offer_price"=>$offer_price,"auction_start_date" => '', "auction_end_date" => '',
						"auction_actual_start_datetime" => '', "auction_actual_end_datetime" => '',"post_date"=>date('Y-m-d H:i:s'));
						}
         ...
         elseif($choose_fixed_weekly=='weekly'){
             if(isset($auction_week) && $auction_week!=''){
				 ...
				 $rowPoster = $obj->selectData(TBL_POSTER, array('fk_user_id'), array("poster_id" => $poster_id));
				 
				 $rowPosterImages = $obj->selectData(TBL_POSTER_IMAGES, array('*'), array("fk_poster_id" => $poster_id));
				 ...
				 $posterData = array('fk_user_id'=>$rowPoster[0]['fk_user_id'],'poster_title' => add_slashes($poster_title), 'poster_desc' => add_slashes($poster_desc), 'flat_rolled' => $flat_rolled,'post_date' => date("Y-m-d H:i:s"),'up_date' => date("Y-m-d H:i:s"),'status' => 1,'post_ip' => $_SERVER["REMOTE_ADDR"],'poster_sku' => generatePassword(6));
				 $posterWhere = array("poster_id" => $poster_id);
				 $new_poster_id=$obj->updateData("tbl_poster_live", $posterData);
	
				 $auctionData = array("fk_auction_type_id"=>'2',"fk_auction_week_id" => $auction_week,"fk_poster_id"=>$new_poster_id,"auction_is_approved"=>'1', "auction_asked_price" => $weekly_asked_price, "auction_reserve_offer_price"=>'',
                     "auction_start_date" => $start_date, "auction_end_date" => $end_date,
                     "auction_actual_start_datetime" => $start_date, "auction_actual_end_datetime" => $end_date);
				 ...
				 for($k=0;$k<count($rowPosterImages);$k++){
				 	
				 	mysqli_query($GLOBALS['db_connect'],"Insert into tbl_poster_images_live (fk_poster_id,poster_thumb,poster_image,is_default,FileExtention,original_filename,is_cloud)  Values ($new_poster_id,'".addslashes($rowPosterImages[$k]['poster_thumb'])."','".addslashes($rowPosterImages[$k]['poster_thumb'])."','".$rowPosterImages[$k]['is_default']."','".$rowPosterImages[$k]['FileExtention']."','".addslashes($rowPosterImages[$k]['original_filename'])."','".addslashes($rowPosterImages[$k]['is_cloud'])."') ");
				 }
                 $obj->deleteData(TBL_BID, array('bid_fk_auction_id' => $auction_id));
                 $obj->deleteData(TBL_AUCTION, array('auction_id' => $auction_id));
				 $obj->deleteData(TBL_POSTER, array('poster_id' => $poster_id));
                 $_SESSION['adminErr'] = "Auction has been re-opened successfully.";
                 header("location: ".$decode_string);
             }else{
...
######  Not required as now bulk upload only for fixed #######	
    for($c=1; $c<$totalItemInArray; $c++)
    {
		...
		}elseif(isEmpty($data2DArray[$c][1])){// Checking for Seller			
			$errCounter++;
			$posterLog .= "Poster {$c}: Seller is empty.<br />";
		}
		######  Not required as now bulk upload only for fixed #######
...
function insertRecord($csvDataArray, $categoryIdArray, $auctionType, $pathOfCsvFile, $user_new_id='', $auctionWeekId='', $auctionWeekStartDate='', $auctionWeekEndDate='', $eventId='', $eventStartDate='', $eventEndDate=''){
    ...
    if (!empty($csvDataArray) && $debug == false) {
		...
		$insertedPosterId = $auctionObj->updateData("tbl_poster_live", $posterArray);
        ...
        if ($insertedPosterId > 0) {
            ...
            for($i = 0; $i < count($posterImageFromCSV); $i++){
					...
					$pos = strpos($posterImageFromCSV[$i], 'default');
					if ($pos === false) {
						echo "The string '$findme' was not found in the string '$mystring'.";
					}
					else {
						$defaultImg = $posterImageFromCSV[$i];
					}					
				...
				$destThumb_buy_gallery = "../poster_photo/thumb_buy_gallery";
				$destThumb_big_slider = "../poster_photo/thumb_big_slider";
				dynamicPosterUpload($posterImageFromCSV,$insertedPosterId,$defaultImg,$src_temp,$dest_poster_photo,$destThumb,$destThumb_buy,$destThumb_buy_gallery,$destThumb_big_slider,"weekly");
            ...
            for($i = 0; $i<count($categoryIdArray); $i++){
				...
				$auctionObj->updateData("tbl_poster_to_category_live", $posterCatAttributes);
			...
			//////
			$row = $auctionObj->selectData(TBL_AUCTION_WEEK, array('*'), array('auction_week_id' => $csvDataArray[9]));
			$start_date=date('Y-m-d',strtotime($row[0]['auction_week_start_date']));
			$end_date=date('Y-m-d',strtotime($row[0]['auction_week_end_date']));
            $startingPrice = 10;
            ...
            $auctionDataArray = array(
								'fk_auction_type_id'                => 2,
								'fk_poster_id'                      => $insertedPosterId,
								'fk_auction_week_id'                => $csvDataArray[9],
								...
								'auction_actual_start_datetime'     => $row[0]['auction_week_start_date'],
								'auction_actual_end_datetime'       => $row[0]['auction_week_end_date'],
								'auction_is_approved'               => 1,
								...
								'status'                            => 1,
								'post_ip'                           => $_SERVER['REMOTE_ADDR'],
								'imdb_link'							=> add_slashes($csvDataArray[11]),
								'up_date'							=> date("Y-m-d h:i:s"),
            );
			
			return $auctionID = $auctionObj->updateData("tbl_auction_live", $auctionDataArray);
...
function reopen_stills(){
 ...
 function set_as_big_slider(){
	...
	$chkLive=$obj->isLiveAuctionItem($_REQUEST['auction_id']);	
...
function manage_invoice_seller_print_bulk(){
	...
	require_once INCLUDE_PATH."lib/adminCommon.php";
	$invoiceObj = new Invoice();
	$invoiceArrr = $_REQUEST['invoice_Arr'];
	$invoiceArr = explode(',', $invoiceArrr);
	
	$i=0;
	$invoiceDataArr=array();
	foreach ($invoiceArr as $value) {
		$invoiceData = $invoiceObj->selectData(TBL_INVOICE,array('*'),array('invoice_id'=>$value));
		$chk_item_type=$invoiceObj->chk_item_type($value);											
		$invoiceData[0]['shipping_address'] = preg_replace_callback('!s:(\d+):"(.*?)";!s', function($m) { return 's:'.strlen($m[2]).':"'.$m[2].'";'; }, $invoiceData[0]['shipping_address']);
		$invoiceDataArr[$i]['shipping_address'] = unserialize($invoiceData[0]['shipping_address']);
		$invoiceData[0]['billing_address'] = preg_replace_callback('!s:(\d+):"(.*?)";!s', function($m) { return 's:'.strlen($m[2]).':"'.$m[2].'";'; }, $invoiceData[0]['billing_address']);
		$invoiceDataArr[$i]['billing_address'] = unserialize($invoiceData[0]['billing_address']);
		$invoiceData[0]['auction_details'] = preg_replace_callback('!s:(\d+):"(.*?)";!s', function($m) { return 's:'.strlen($m[2]).':"'.$m[2].'";'; }, $invoiceData[0]['auction_details']);
		$invoiceDataArr[$i]['auction_details'] = unserialize($invoiceData[0]['auction_details']);
		$invoiceData[0]['additional_charges'] = preg_replace_callback('!s:(\d+):"(.*?)";!s', function($m) { return 's:'.strlen($m[2]).':"'.$m[2].'";'; }, $invoiceData[0]['additional_charges']);
		$invoiceDataArr[$i]['additional_charges'] = unserialize($invoiceData[0]['additional_charges']);
		$invoiceData[0]['discounts'] = preg_replace_callback('!s:(\d+):"(.*?)";!s', function($m) { return 's:'.strlen($m[2]).':"'.$m[2].'";'; }, $invoiceData[0]['discounts']);
		$invoiceDataArr[$i]['discounts'] = unserialize($invoiceData[0]['discounts']); 
		$user_email = $invoiceObj->selectData(USER_TABLE,array('email'),array('user_id'=>$invoiceData[0]['fk_user_id']));
		$invoiceDataArr[$i]['user_email']=$user_email;
		$invoiceDataArr[$i]['invoice_generated_on']=$invoiceData[0]['invoice_generated_on'];
		$invoiceDataArr[$i]['is_buyers_copy']=$invoiceData[0]['is_buyers_copy'];
		$invoiceDataArr[$i]['is_approved']=$invoiceData[0]['is_approved'];
		$invoiceDataArr[$i]['is_cancelled']=$invoiceData[0]['is_cancelled'];
		$invoiceDataArr[$i]['is_paid']=$invoiceData[0]['is_paid'];
		$i++;
	}
	
	$smarty->assign("invoiceData", $invoiceDataArr);
	//echo"<pre>".print_r($invoiceDataArr)."</pre>";
  	
	$smarty->display('admin_manage_invoice_seller_print_bulk.tpl');
	}
function compareByName($a, $b) {
  return strcmp($a["poster_title"], $b["poster_title"]);
}
?>