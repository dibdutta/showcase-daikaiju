...
if($mode == "user_report"){
	user_report_display();
}elseif($mode == "print_user_report"){
	print_user_report();
}elseif($mode == "auction_report"){
	auction_report_display();
}elseif($mode == "print_auction_report"){
	print_auction_report();
}elseif($mode == "pay_now_seller"){
	pay_now_seller();
}elseif($mode == "complete_payment"){
	complete_payment();
}elseif($mode == "admin_auction_seller_detail"){
	admin_auction_seller_detail();
}elseif($mode == "auction_payment_report"){
	auction_payment_report();
}elseif($mode == "print_seller_payment_report"){
...
function auction_report_display(){
    ...
    if(isset($_REQUEST['end_date']) && $_REQUEST['end_date']!='' && compareDates($_REQUEST['end_date'],($_REQUEST['start_date'] ?? ''))){
		...
		$total_amount_unpaid_by_buyer = 0;
		if(($_REQUEST['auction_week'] ?? '')!='' or ($_REQUEST['auction_type'] ?? '')=='fixed'){
        ...
        $smarty->assign('total_unapproved', $total_unapproved ?? 0);
        $smarty->assign('total_own', number_format(($total_own), 2, '.', ''));
        $smarty->assign('total_pending', $total_pending ?? 0);
        $smarty->assign('total_unsold', $total_unsold);
        $smarty->assign('total_reopen', $total_reopen ?? 0);
        $smarty->assign('total_upcoming', $total_upcoming ?? 0);
        $smarty->assign('total_selling', $total_selling ?? 0);
...
if(($_REQUEST['search'] ?? '')!='reconciliation'){
 ...
 function complete_payment(){
	...
	if(($_REQUEST['auction_type'] ?? '')=='stills'){
 	...
 	if(($_REQUEST['user_id'] ?? '')!=''){
 	if(isset($_REQUEST['auction_id']) && count($_REQUEST['auction_id'])>0){
 		$sql_insert="Insert into `tbl_mpe_admin_payment_to_seller` (user_id,payment_amount,payment_date,auction_week_id) values ('".($_REQUEST['user_id'] ?? '')."','".($_REQUEST['pay_amnt'] ?? 0)."','".date('Y-m-d',strtotime($_REQUEST['start_date'] ?? ''))."','".($_REQUEST['auction_week'] ?? '')."')";
