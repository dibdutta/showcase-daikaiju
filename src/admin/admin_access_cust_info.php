...
if($_REQUEST['mode'] == "buyer"){
	access_buyer_info();
}elseif($_REQUEST['mode'] == "seller"){
	access_seller_info();
}elseif($_REQUEST['mode'] == "fetch_invoice"){
	fetch_invoice();
}elseif($_REQUEST['mode'] == "mark_shipping_claimed"){
    ...
    function access_buyer_info(){
        ...
        if($_REQUEST['start_date'] > $_REQUEST['end_date']){
            ...
            $total = $auctionObj->countFixedPriceSaleByStatus('',$seller_user_id,$_REQUEST['search_fixed_poster'],$start_date,$end_date,$buyer_user_id);
            if($total>0){
                ...
                if(isset($_REQUEST['buyer_user_id']) && $_REQUEST['buyer_user_id']!=''){
                    ...
                    $total_selling = $auctionObj->countFixedPriceSaleByStatus('selling',$seller_user_id,$_REQUEST['search_fixed_poster'],$start_date,$end_date,$buyer_user_id);
                    $smarty->assign('total_selling', $total_selling);
                }
                $total_pending = $auctionObj->countFixedPriceSaleByStatus('pending',$seller_user_id,$_REQUEST['search_fixed_poster'],$start_date,$end_date,$buyer_user_id);
                $total_sold = $auctionObj->soldAuctionCOUNTFIXED('sold',$seller_user_id,$_REQUEST['search_fixed_poster'],$start_date,$end_date,$buyer_user_id);
                $total_unpaid = $auctionObj->countFixedPriceSaleByStatus('unpaid',$seller_user_id,$_REQUEST['search_fixed_poster'],$start_date,$end_date,$buyer_user_id);
                $total_paid = $auctionObj->countFixedPriceSaleByStatus('paid',$seller_user_id,$_REQUEST['search_fixed_poster'],$start_date,$end_date,$buyer_user_id);
                $total_pending_invoiced = $auctionObj->countFixedPriceSaleByStatus('pending_invoice',$seller_user_id,$_REQUEST['search_fixed_poster'],$start_date,$end_date,$buyer_user_id);
            }
	...
	function fetch_invoice(){
		...
		$invoiceObj = new Invoice();
		$invoiceData = $invoiceObj->fetchInvoiceAuctionDetails($_REQUEST['invoice_id']);
