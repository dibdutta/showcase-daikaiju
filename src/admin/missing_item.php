...
$sql = " SELECT a.auction_id,a.max_bid_amount,p.poster_title FROM tbl_auction a,tbl_poster p WHERE a.fk_auction_week_id = ".($_REQUEST['week_id'] ?? '')." AND a.auction_is_sold='1' AND
...
while($row = mysqli_fetch_array($res)){
	...
	if($row_archive['counter']==0){
		...
		$res_archive_bids = mysqli_query($GLOBALS['db_connect'],$sql_archive_bids);
		$row_archive_bids = mysqli_fetch_array($res_archive_bids);
		echo "Missing-".$row['poster_title']."(".$row['auction_id'].")";
		echo "<br/>";
