...
function myFunction(){
    ...
    if(check) {
        $.post("admin_manage_auction_week.php?mode=update_auction_status", { auction_week_id: auction_week_id, val: '1'},
		function(data){
			alert('Auction is successfully shifted to hidden mode')
		});
    } else {
        $.post("admin_manage_auction_week.php?mode=update_auction_status", { auction_week_id: auction_week_id, val: '0'},
		function(data){
	...
	<tr>
		<td width="100%" align="center">
			<form name="frm_add_event" id="frm_add_event" method="post" action="">
                ...
                <input type="hidden" name="auction_week_id" value="{$auction_week[0].auction_week_id}" />
