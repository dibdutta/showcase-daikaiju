...
//	}
	...
	$total_paid=$objAuction->countTotalAuctionsAdmin($_REQUEST['user_id'],'paid',$start_date,$end_date);
	
	$objAuction->offset = 0;
	$objAuction->toShow = $total_paid;
	
	$FetchTotalpaid=$objAuction->fetchTotalAuctionsAdmin($_REQUEST['user_id'],'paid',$start_date,$end_date);
	$FetchTotalpaid=$objAuction->fetchSoldPriceForSeller($FetchTotalpaid);
	$total_sold_price=0;
	$total_fee=0;
	$total_own=0;
	
	for($i=0;$i<$total_paid;$i++){
		$FetchTotalpaid[$i]['mpe_charge']=number_format(($FetchTotalpaid[$i]['soldamnt']* MPE_CHARGE_TO_SELLER)/100, 2, '.', '');
		$FetchTotalpaid[$i]['trans_charge']=number_format(($FetchTotalpaid[$i]['soldamnt']* MPE_TRANSACTION_CHARGE_TO_SELLER)/100, 2, '.', '') ;
		$FetchTotalpaid[$i]['tot_charge']=$FetchTotalpaid[$i]['mpe_charge']+$FetchTotalpaid[$i]['trans_charge'];
		$FetchTotalpaid[$i]['seller_owned']=number_format(($FetchTotalpaid[$i]['soldamnt']-($FetchTotalpaid[$i]['mpe_charge']+ $FetchTotalpaid[$i]['trans_charge'])), 2, '.', '');
		$total_sold_price=$total_sold_price + $FetchTotalpaid[$i]['soldamnt'];
		$total_fee=$total_fee + $FetchTotalpaid[$i]['tot_charge'];
		$total_own=number_format(($total_own + $FetchTotalpaid[$i]['seller_owned']), 2, '.', '');
	}
	...
	$smarty->assign('paidAuctionDetails', $FetchTotalpaid);
...
function print_auction_report()
{
	...
	$objAuction = new Auction();
	if($_REQUEST['search']!='reconciliation'){
	$dataAuction=$objAuction->fetchTotalAuctionsAdmin($_REQUEST['user_id'],$_REQUEST['search'],$_REQUEST['start_date'],$_REQUEST['end_date']);	
	$dataAuction=$objAuction->fetchWinnerAndSoldPrice($dataAuction);
	$smarty->assign('dataAuction', $dataAuction);
	}
...
if($_REQUEST['search']!='reconciliation'){
	...
	$FetchTotalpaid=$objAuction->fetchTotalAuctionsAdmin($_REQUEST['user_id'],'paid',$start_date,$end_date);
	$FetchTotalpaid=$objAuction->fetchSoldPriceForSeller($FetchTotalpaid);
	//print_r($FetchTotalpaid);
	$total_sold_price=0;
	$total_fee=0;
	$total_own=0;
	for($i=0;$i<$total_paid;$i++){
		//$FetchTotalpaid[$i]['sold_price']=$objAuction->sellerInvoicePrice($FetchTotalpaid[$i]['auction_id']);
		$FetchTotalpaid[$i]['mpe_charge']=number_format(($FetchTotalpaid[$i]['soldamnt']* MPE_CHARGE_TO_SELLER)/100, 2, '.', '');
		$FetchTotalpaid[$i]['trans_charge']=number_format(($FetchTotalpaid[$i]['soldamnt']* MPE_TRANSACTION_CHARGE_TO_SELLER)/100, 2, '.', '') ;
		$FetchTotalpaid[$i]['tot_charge']=$FetchTotalpaid[$i]['mpe_charge']+$FetchTotalpaid[$i]['trans_charge'];
		$FetchTotalpaid[$i]['seller_owned']=number_format(($FetchTotalpaid[$i]['soldamnt']-($FetchTotalpaid[$i]['mpe_charge']+ $FetchTotalpaid[$i]['trans_charge'])), 2, '.', '');
