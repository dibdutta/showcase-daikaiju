...
function myAyctions()
{
    ...
    for($i=0;$i<$total_now;$i++)
    {
		...
		if ($auctionRow[$i]['is_cloud'] !='1'){
                ...
                $auctionRow[$i]['image_path']=CLOUD_POSTER_THUMB.$auctionRows[$i]['poster_thumb'];
            }else{
...
//    echo "</pre>";
    ...
    if($error_ind == 0){
    	for($c=1; $c<$totalItemInArray; $c++)
    	{
			...
			$sizeId = fetchCategoryId($size,1);
			$genreId = fetchCategoryId($genre,2);
    	 	$auctionID = insertRecord($data2DArray[$c], array($conditionId, $sizeId, $genreId),$data2DArray[$c][$innerElement] ,$pathOfCsvFile);
    	}		
				if($auctionID > 0){
					$successCounter++;
