...
function displayList()
{
	...
	if($list == 'upcoming'){
	  ...
	  if(!isset($_REQUEST['track_is_expired'])){
		...
		if($list=='weekly' && $total==0 && $auctionWeeks==0){
			...
			if($total == 0 && $upcomingTotal > 0){
				header("location: buy?list=upcoming");
				exit;
			}
	...
	$smarty->assign('displayCounterTXT', displayCounter($total, $GLOBALS['offset'], $GLOBALS['toshow'], "headertext", $message="", $start=44, $end=132, $step=44, $use=1));
...
if(isset($_SESSION['sessUserID'])){
	...
	// Outbid email links carry &live=1 to mark them as tbl_auction_live IDs.
	// After the auction ends, cron deletes from tbl_auction_live and writes the
	// old→new ID into tbl_auction_mapping. Redirect to the correct sold view so
	// the user sees their original item and not a colliding tbl_auction record.
	if(!empty($_REQUEST['live'])){
    ...
    if ($auctionDetails[0]['is_cloud']==1){
        ...
        $auctionDetails[0]['image_path']=CLOUD_POSTER_THUMB_BUY_GALLERY.$auctionDetails[0]['poster_thumb'];
        $auctionDetails[0]['large_image']=CLOUD_POSTER.$auctionDetails[0]['poster_thumb'];
        $auctionDetails[0]['disp_image']=CLOUD_POSTER_THUMB_BUY_GALLERY.$auctionDetails[0]['poster_thumb'];
