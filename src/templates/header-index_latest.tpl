...
<link href="https://d2m46dmzqzklm5.cloudfront.net/css/template_new.css" rel="stylesheet" type="text/css" />
...
function showTimer1(list,id){
  if(document.getElementById('timer1')){
	  ...
	  $.get('ajax?mode=chk_auction_week&week_id='+id, function(data) {
	   if(data=='1'){
        ...
        <!--<div class="search-left-bg"></div>-->
            <div class="search-midrept-bg ">
				...
				{if $smarty.request.list != 'stills' && $smarty.request.mode != 'refinesrcStills'}
					<input type="button" value="" class="refine-srchbtn-main" onclick="$(location).attr('href', '{$actualPath}/buy?mode=refinesrc&list={$smarty.request.list}&auction_week_id={$smarty.request.auction_week_id}');" />
				{else}
				    <input type="button" value="" class="refine-srchbtn-main" onclick="$(location).attr('href', '{$actualPath}/buy?mode=refinesrcStills');" />
				{/if}
            </div>
...
<div style="z-index:1000021; border:4px solid #881319; position:fixed;background-color:#FFFFFF; color:#OOO; width:700px; height:80px; padding:10px 20px; display:none;top: 50%; left: 50%; margin-top: -100px; margin-left: -250px;" id="reset_warning1">
        ...
        <div style="height:20px; text-align:center; padding:10px 0px; font-size:14px;">Thank you for bidding! You wil be now redirected to Extended Auction Section.<br/>
                                                                                    Sold items results are being collated and will be published in a few minutes.<br/>
