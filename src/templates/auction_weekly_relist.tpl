...
<script type="text/javascript" charset="utf-8">
	...
	function validate_poster_fixed(){
		var auction_id = $("#auction_id").val();
		var buynow_price = $("#buynow_price_fixed").val();
    	var chk_ind=/^ *[0-9]+ *$/.test(buynow_price);
		var asked_price = $("#asked_price_fixed").val();
		var chk_ind_asked=/^ *[0-9]+ *$/.test(asked_price);
		var is_percentage=$("#is_percentage").val();
		if(asked_price==''){
			alert("Please enter value for Asked Price");
			...
			alert("Please enter integer value for Asked Price");
    	 	$('#asked_price_fixed').val("");
    	 	return;
		}else if(buynow_price!='' && chk_ind==false){
			alert("Please enter integer value for Offer Price");
    	 	$('#buynow_price_fixed').val("");
    	 	return;
		}else if(chk_ind==true){
			if($('#is_percentage').attr('checked')){
				if(asked_price <= (asked_price * buynow_price/100)){
					alert("Offer price must be less than Asked price.");
					$('#buynow_price_fixed').val("");
				}else{
					$.get('myselling.php',{ mode: "relist_weekly_to_fixed", auction_id: auction_id,asked_price:asked_price,is_percentage:1,buynow_price:buynow_price },
					 function(data) {
					$(".list-auction-fixed-ask").hide(); 
					$(".Note").hide();
					$("#formarea").hide(); 
			...
			}else if(parseInt(buynow_price) >= parseInt(asked_price)){
				alert(" Offer price must be less than Asked price.");
        		$('#buynow_price_fixed').val("");
        		return;
			 }else{
			$.get('myselling.php',{ mode: "relist_weekly_to_fixed", auction_id: auction_id,asked_price:asked_price,is_percentage:0,buynow_price:buynow_price},
			 function(data) {
			$(".list-auction-fixed-ask").hide();
			$(".Note").hide(); 
			$("#formarea").hide();
...
<div class="MoveToWeeklyPopUp">
  ...
  <input type="hidden" name="auction_id" id="auction_id" value="{$auction_id}" />
    <div class="per-field">
      ...
      .00 </div>
    <div class="per-field">
      <label for="firstname"> <strong>Minimum offer you will consider :</strong></label>
      $
      <input type="text" class="register-txtfield required" size="32"  maxlength="8" value="" name="buynow_price" id="buynow_price_fixed">
      .00 </div>
	  <!--<div class="activity bold">Amount as a percentage
	  <input type="checkbox"  value="1" name="is_percentage" id="is_percentage" >
	  </div>-->
    <div class="per-field">
      <label></label>
