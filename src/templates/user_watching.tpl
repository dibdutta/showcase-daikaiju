...
<div><a href="#"><img  class="image-brdr"  src="{$watchingItems[counter].image_path}"  onclick="redirect_poster_details({$watchingItems[counter].auction_id});" style="cursor:pointer;"  /></a>
			...
			test_enter(auc_id);
										  ...
										  <td><div>
											<input type="button" id="offer_bttn_{$watchingItems[counter].auction_id}" value="MAKE OFFER" onclick="postOffer({$watchingItems[counter].auction_id}, '{$watchingItems[counter].fk_user_id}','{$watchingItems[counter].auction_asked_price}');" class="bidnow-btn makeoffer" style="margin:1px 0 0 0;" />
											</div></td>
										  </tr>
										</table>
                                        </div>
