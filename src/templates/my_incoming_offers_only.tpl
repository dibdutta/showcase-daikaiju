...
$(document).ready(function(){
	...
	setInterval(function() { refreshIncomingOffersOnly(offset,toshow,orderBy,orderType); }, 50000);
})
</script>
<script type="text/javascript">
                    ...
                    {if $total > 0}
                        ...
                        {section name=counter loop=$auctionRows}
                            <tr>
                                ...
                                <td width="18%" valign="top">
									<div id="gallery_{$smarty.section.counter.index}" class="posterdetails">
                                     <table align="center"><tbody><tr><td align="left" valign="top" style="border:none; vertical-align:top;">
											<div class="buylisttb">
                                       <div><img  class="image-brdr" src="{$auctionRows[counter].image_path}" border="0"  border="0" onclick="redirect_poster_details({$auctionRows[counter].auction_id}, 1);" style="cursor:pointer;" /></div></div>
                                        </td></tr></tbody></table>
									</div>
                                </td>
                                ...
                                <p class="poster-txt">
                                {$auctionRows[counter].poster_desc}
                                </p>
                                </td>
                            ...
                            <tr>
                                <td colspan="5">
                                    ...
                                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                                        <tr>
                                            <th width="4%" class="text bold">Offer</th>
                                            <th width="28%" class="text bold">Offer's Status</th>
                                            <th width="4%" class="text bold">Offer Date</th>
                                            <th width="26%" class="text bold">Counter Offer</th>
                                            <th width="6%" class="text bold">Counter Offer Status</th>
                                            <th width="6%" class="text bold">Counter Offer Date</th>
                                    	</tr>
                                        {section name=offerCounter loop=$offers}                                   
                                        <tr>
                                            <td style="text-align:center;" class="text">${$offers[offerCounter].offer_amount}</td>
                                            <td id="ofr_status_{$offers[offerCounter].offer_id}" style="text-align:center;" >
                                            {if $offers[offerCounter].offer_is_accepted == 0}

												<div style="margin:0 auto;"><input type="button" value="Accept" class="track-btn-small" style="margin: 5px 2px" onclick="acceptOfferNew({$auctionRows[counter].auction_id}, {$offers[offerCounter].offer_id},{$offers[offerCounter].offer_id}, 'seller',{$offset},{$toshow});" /></div>
												<div style="margin:0 auto;"><input type="button" value="Reject" class="track-btn-small" style="margin: 0 2px 5px 2px" onclick="rejectOfferNew({$offers[offerCounter].offer_id},{$offers[offerCounter].offer_id}, 'seller',{$offset},{$toshow});" /></div>
											...
											<td id="cntr_ofr_{$offers[offerCounter].offer_id}"  style="text-align:center;">
                                            {if $offers[offerCounter].cntr_offer_id > 0}
												${$offers[offerCounter].cntr_ofr_offer_amount}
											{elseif $offers[offerCounter].offer_is_accepted != 2}
												{if $auctionRows[counter].auction_is_sold == 0}
                                                    <div style="width:135px; margin:0 auto;"><div class="fll" style="width:10px;">$</div><input type="text" id="cntr_amt_{$offers[offerCounter].offer_id}" class="inner-txtfld" style="width:107px; height:15px; margin-left:5px; float:left;" /></div><div><input type="button" value="Counter Offer" class="track-btn-small" style="margin: 5px 2px 5px 16px" onclick="makeCounterOfferModified({$auctionRows[counter].auction_id}, {$offers[offerCounter].offer_id}, $('#cntr_amt_'+{$offers[offerCounter].offer_id}).val(),{$offset},{$toshow});" /> </div>
												{else}
													--
												{/if}
											{/if}
