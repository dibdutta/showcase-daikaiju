            ...
            <div id="tabbed-inner-nav">
				...
				<div class="innerpage-container-main">
					...
					{if $errorMessage<>""}<div class="messageBox">{$errorMessage}</div>{/if}
                    	{if $total > 0}
							...
							<div class="left-area">
									...
									{if $smarty.request.mode=='search_sold'}
										<li class="list"><a href="sold_item?view_mode=list&mode=search_sold&search_sold={$smarty.request.search_sold|urlencode}">&nbsp;</a> </li>
									{else}
                                        <li class="list"><a href="sold_item?view_mode=list">&nbsp;</a> </li>
									{/if}	
                                        |
                        ...
                        <input type="hidden" value="search_sold" name="mode" >
                        ...
                        <input type="hidden" value="{$smarty.request.toshow}" name="toshow" >
                            ...
                            <div style="width:270px; height:26px;border:1px solid #cecfd0; float:left;">
                            	...
                            	<input type="text" style="width:232px; height:23px;border:0px solid #cecfd0; padding:3px 0 0 0;" class="midSearchbg_edit fll" id="search_sold" name="search_sold" {if $smarty.request.mode == 'search_sold'} value="{$smarty.request.search_sold}" {else} value="Search sold items by their title.."{/if} onfocus="clear_text()"  />
                            ...
                            <div class="btomgrey-bg"></div>                 
                                {section name=counter loop=$dataJstFinishedAuction}	
                                    ...
                                    <div {if $smarty.session.sessUserID == ""} class="grid-view-main gridMrgn" {else} class="grid-view-main " {/if}>
                                       ...
                                       <div class="pb05 pl10 pr10 tac" style="height:40px;"><h3><a class="gridView" href="{posterUrl id=$dataJstFinishedAuction[counter].auction_id title=$dataJstFinishedAuction[counter].poster_title}" style="cursor:pointer;" >{$dataJstFinishedAuction[counter].poster_title}&nbsp;{*if $smarty.session.sessUserID <> ""}(#{$dataJstFinishedAuction[counter].poster_sku}){/if*}</a></h3></div>	
                                                </div>                                            
