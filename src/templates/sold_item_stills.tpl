                      ...
                      {if $total > 0}
							...
							<div class="left-area_auction" style="width:150px;">
									...
									{if $smarty.request.mode=='search_sold_stills'}
										<li class="grid"><a href="sold_item?view_mode=grid&mode=search_sold_stills&search_sold={$smarty.request.search_sold|urlencode}">&nbsp;</a> </li>
									{else}
                                        <li class="grid"><a href="sold_item?view_mode=grid&mode=stills">&nbsp;</a> </li>
									{/if}	
                        ...
                        <input type="hidden" value="{$smarty.request.toshow}" name="toshow" >
							 ...
							 <div class="soldSearchblock" style="clear:right;">
                            	...
                            	<input style="width:450px; height:23px;border:0px solid #cecfd0; padding:3px 0 0 5px;" type="text" class="midSearchbg_edit fll" id="search_sold" name="search_sold" {if $smarty.request.mode == 'search_sold_stills'} value="{$smarty.request.search_sold}" {else} value="Search sold items by their title.."{/if} onfocus="clear_text()"  />
							...
							<div class="sortblock_auction2" style="clear: both; width: 290px; margin-top:8px; margin-left:0; text-align:left;" ><span class="headertext">&nbsp;</span><b>Sort By:</b>
                             <select name="auction_week" class="look" onchange="javascript: this.form.submit();">
                                 ...
                                 {section name=counter loop=$auctionWeek}
                                     <option value="{$auctionWeek[counter].auction_week_id}" {if $smarty.request.auction_week == $auctionWeek[counter].auction_week_id} selected {/if} >Kaijulink Stills Auction&nbsp;( {$auctionWeek[counter].auction_week_end_date|date_format:'%D'})</option>
                                 {/section}
                             </select>
							</div>
                         	<div class="sortblock" style="margin-top: 8px;">{$displaySortByTXT}</div>
