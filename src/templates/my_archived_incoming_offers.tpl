                            ...
                            <table class="list-bid-det-main" cellpadding="0" cellspacing="0" border="0">
                                {section name=counter loop=$auctionRows}
                                    <tr>
                                        ...
                                        <td width="25%" valign="top" style="padding:0 15px;"><p class="poster-txt">{$auctionRows[counter].poster_desc}</p></td>
                                                ...
                                                <div style="margin:0; border-bottom:1px solid #DFDFDF;">
                                                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
														<tr>
                                                            ...
                                                            <th width="13%" class="text bold">Offer's Status</th>
                                                            <th width="13%" class="text bold">Offer Date</th>
                                                            <th width="13%" class="text bold">Counter Offer</th>
                                                            <th width="30%" class="text bold">Counter Offer's Status</th>
                                                            <th width="13%" class="text bold">Counter Offer Date</th>
														</tr>
														{section name=offerCounter loop=$offers}                                   
														<tr>
															<td align="left" class="text">${$offers[offerCounter].offer_amount}</td>
															<td id="ofr_status_{$offers[offerCounter].offer_id}" align="left" class="text">
															{if $offers[offerCounter].offer_is_accepted == 0}

																<div style="float:left; padding-right:10px"><input type="button" value="Accept" class="track-btn-small" onclick="acceptOfferNew({$auctionRows[counter].auction_id}, {$offers[offerCounter].offer_id},{$offers[offerCounter].offer_id}, 'seller',{$offset},{$toshow});" /></div>
																<div style="float:left; padding-right:10px"><input type="button" value="Reject" class="track-btn-small" onclick="rejectOfferNew({$offers[offerCounter].offer_id},{$offers[offerCounter].offer_id}, 'seller',{$offset},{$toshow});" /></div>
                                                            ...
                                                            <td class="text">{$offers[offerCounter].post_date|date_format:"%m/%d/%Y"}</td>
															<td id="cntr_ofr_{$offers[offerCounter].offer_id}" align="left" class="text">
															{if $offers[offerCounter].cntr_offer_id > 0}
																${$offers[offerCounter].cntr_ofr_offer_amount}
															{else}
																{if $auctionRows[counter].auction_is_sold == 0}
																<!--<div style="float:left; padding-right:20px;">$<input type="text" id="cntr_amt_{$offers[offerCounter].offer_id}" class="inner-txtfld" /></div><div><input type="button" value="Counter Offer" class="track-btn-small" onclick="makeCounterOfferModified({$auctionRows[counter].auction_id}, {$offers[offerCounter].offer_id}, $('#cntr_amt_'+{$offers[offerCounter].offer_id}).val(),{$offset},{$toshow});" /></div> -->
																    --
																{else}
																	--
																{/if}
                                                            ...
                                                            <td class="text">{if $offers[offerCounter].cntr_offer_id > 0} {$offers[offerCounter].cntr_ofr_post_date|date_format:"%m/%d/%Y"}{else} --{/if}</td>
														</tr>
														{/section}
													</table>
                                                </div>
