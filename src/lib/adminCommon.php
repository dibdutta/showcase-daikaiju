<?php
	...
	$smarty->assign('adminLastLoginDate', date("dS M, Y h:i:s:A", strtotime($_SESSION['adminLastLoginDate'])));
	$smarty->assign('adminCurrentDate', date("dS M, Y h:i:s:A"));
	
	if($_SESSION['adminErr']=="" && $_REQUEST['msg']!=""){
		$_SESSION['adminErr'] = $_REQUEST['msg'];
	}
	$smarty->assign('errorMessage', $_SESSION['adminErr']);
	$_SESSION['adminErr'] = "";

	if(basename($_SERVER['PHP_SELF'])!="admin_main.php" && basename($_SERVER['PHP_SELF'])!="admin_login.php" && $_SESSION['superAdmin']!=1 && MULTIUSER_ADMIN == true){
