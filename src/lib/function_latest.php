...
function checkEmail($emailString,$labelEmail){
    ...
    {
          $emailStringArray=split(",",$emailString);
          $labelEmailArray=split(",",$labelEmail);
    ...
    if($total > $toshow) {
        ...
        for($ii=0; $ii<ceil($total/$toshow); $ii++) {
            if(($ii+1) <= 0 ) {
                ...
                $link = DOMAIN_PATH.$_SERVER["PHP_SELF"].generateLink(($ii*$toshow), $toshow, $GLOBALS['order_by'], $GLOBALS['order_type'])."";
                if($offset == ($ii*$toshow) ) {
...
function displayCounter($total,$offset,$toshow,$class, $message="Reports Per Page.", $start=5, $end=100, $step=5, $use=1) {
    ...
    for ($i=$start; $i <=$end; $i=$i+$step){
        $link = DOMAIN_PATH.$_SERVER['PHP_SELF'].generateLink(0, $i, $GLOBALS['order_by'], $GLOBALS['order_type'])."";
        if ($toshow == $i){
...
function displaySortBy($class) {
	...
	if ($_REQUEST['list']=="weekly" && $_SERVER['PHP_SELF']=="/buy.php" && $_SERVER['PHP_SELF']!="/offers.php"){
        $link = DOMAIN_PATH.$_SERVER['PHP_SELF'].generateLink($GLOBALS["offset"], $GLOBALS["toshow"], 'auction_bid_price', 'ASC');
        ...
        $link = DOMAIN_PATH.$_SERVER['PHP_SELF'].generateLink($GLOBALS["offset"], $GLOBALS["toshow"], 'auction_asked_price', 'ASC');
    ...
    if ($_REQUEST['list']=="weekly" && $_SERVER['PHP_SELF']=="/buy.php" && $_SERVER['PHP_SELF']!="/offers.php"){
        $link = DOMAIN_PATH.$_SERVER['PHP_SELF'].generateLink($GLOBALS["offset"], $GLOBALS["toshow"], 'auction_bid_price', 'DESC');
        ...
        $link = DOMAIN_PATH.$_SERVER['PHP_SELF'].generateLink($GLOBALS["offset"], $GLOBALS["toshow"], 'auction_asked_price', 'DESC');
	...
	$link = DOMAIN_PATH.$_SERVER['PHP_SELF'].generateLink($GLOBALS["offset"], $GLOBALS["toshow"], 'invoice_generated_on', 'ASC');
		...
		$link = DOMAIN_PATH.$_SERVER['PHP_SELF'].generateLink($GLOBALS["offset"], $GLOBALS["toshow"], 'invoice_generated_on', 'DESC');
	...
	{
		$link = DOMAIN_PATH.$_SERVER['PHP_SELF'].generateLink($GLOBALS["offset"], $GLOBALS["toshow"], 'auction_actual_end_datetime', 'ASC');
		...
		$link = DOMAIN_PATH.$_SERVER['PHP_SELF'].generateLink($GLOBALS["offset"], $GLOBALS["toshow"], 'auction_actual_end_datetime', 'DESC');
    ...
    if($_REQUEST['mode']!='premier' && $_REQUEST['mode']!='search_sold_premier'){
        $link = DOMAIN_PATH.$_SERVER['PHP_SELF'].generateLink($GLOBALS["offset"], $GLOBALS["toshow"], 'poster_title', 'ASC');
        ...
        $link = DOMAIN_PATH.$_SERVER['PHP_SELF'].generateLink($GLOBALS["offset"], $GLOBALS["toshow"], 'poster_title', 'DESC');
...
function pageCounter($total, $offset, $toshow, $class, $groupby=10, $showcounter=1, $linkStyle='', $redText='red_text') {
    ...
    if($total>0){
        ...
        if($total > $toshow && ($showcounter==1 || $showcounter==2 || $showcounter==3 || $showcounter==4)) {
            $no_of_pages=ceil($total/$toshow);
            if($offset != 0) {
                $returnTXT .= "<a href='".DOMAIN_PATH.$_SERVER["PHP_SELF"].generateLink(($offset - $toshow), $toshow, $GLOBALS['order_by'], $GLOBALS['order_type'])."' class='".$linkStyle."'>&laquo; Prev</a>&nbsp;";
            }
            for($ii=0; $ii<ceil($total/$toshow); $ii++) {
                if(($ii+1) <= 0 ) {
            ...
            if(($offset + $toshow)<$total) {
                $returnTXT .= " <a href='".DOMAIN_PATH.$_SERVER["PHP_SELF"].generateLink(($offset + $toshow), $toshow, $GLOBALS['order_by'], $GLOBALS['order_type'])."' class='".$linkStyle."'>Next &raquo;</a>&nbsp;";
            }
        }
    }
...
function orderBy($linkname, $orderby, $imageshow=1, $class){
    ...
    $returnTXT .= "<a href=\"".DOMAIN_PATH.$_SERVER['PHP_SELF'].generateLink($_REQUEST['offset'], $_REQUEST['toshow'], $orderby, $GLOBALS['order_type_s'])."\" class=\"".$class."\" >".$linkname."";
    if($imageshow==1){
...
function sendMail($toMail, $toName, $subject, $textContent, $fromMail, $fromName, $html=1,$attachment='') {
    //require_once INCLUDE_PATH."classes/class.phpmailer.php";
    //require_once INCLUDE_PATH."classes/class.smtp.php";
    
    if($_SERVER['HTTP_HOST'] == LOCAL_HOST_NAME){
        return true;
        $sendMail="deep@server";
    }else{
        $sendMail = $toMail;
    }
    $sendName = $toName;
    ...
    if($attachment !=''){
	    	$mail->AddAttachment($attachment,"Bulk Upload.zip");    
	    }     
...
function increment_amount($buy_now){
 ...
 function dynamicPosterUpload($posterArr,$poster_id,$is_default,$src_temp,$dest_poster_photo,$destThumb,$destThumb_buy,$destThumb_buy_gallery){
	...
	foreach($posterArr as $key => $value){
        ...
        $poster_image_id=$obj->updateData(TBL_POSTER_IMAGES, array("fk_poster_id" => $poster_id, "poster_thumb" => addslashes($value),
                                                  "poster_image" => addslashes($value), "is_default" => $set_default,"FileExtention"=>$imageExt,"original_filename"=>addslashes($value),"is_cloud"=>'1'));
		 ...
		 */
		 ...
		 $container = $conn->get_container(CLOUD_POSTER_CONTAINER);
		 $object = $container->create_object($fileName);
		 $object->load_from_filename($originalImage);
		 //echo $object->public_uri();
		
		//Upload thumb image	
		 $container = $conn->get_container(CLOUD_POSTER_THUMB_CONTAINER);
		 $object = $container->create_object($fileName);
		 $object->load_from_filename($thumbImage);
		 //echo $object->public_uri();
				
		//Upload thumbbuy image	
		 $container = $conn->get_container(CLOUD_POSTER_THUMB_BUY_CONTAINER);
		 $object = $container->create_object($fileName);
		 $object->load_from_filename($thumbBuyImage);
		 //echo $object->public_uri();	
				
		//Upload thumbbuy gallery image	
		 $container = $conn->get_container(CLOUD_POSTER_THUMB_BUY_GALLERY_CONTAINER);
		 $object = $container->create_object($fileName);
...
function chkTimeOut(){}
 function clrCartAfterLogout(){
 	$sql_delete = " Delete from ".TBL_CART_HISTORY."  where fk_user_id= '".$_SESSION['sessUserID']."' ";
