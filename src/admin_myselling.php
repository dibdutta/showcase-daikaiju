...
if($_REQUEST['mode'] == "move_to_weekly"){
    move_to_weekly();
...
function myAyctions()
{
	...
	if($status!='sold'){
    	$total = $objAuction->countAuctionsByStatus($_SESSION['sessUserID'], $status,'',$title);
    	$auctionRow = $objAuction->fetchAuctionsByStatus($_SESSION['sessUserID'], $status,$_REQUEST['sort_by'],'',$title);
	}else{
    ...
    {
        if (file_exists("poster_photo/" . $auctionRow[$i]['poster_image'])){
...
function images_next(){
 ...
 function move_to_weekly(){
	...
	$auctionWeekObj = new AuctionWeek();
    $aucetionWeeks = $auctionWeekObj->fetchUpcomingWeeksNewForAdmin();
