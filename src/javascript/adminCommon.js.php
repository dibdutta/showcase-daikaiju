...
function onLoadAdminCommonFunction() {
	...
	highlightsRowsInit();
	MM_preloadImages('<?php echo CLOUD_STATIC_ADMIN; ?>slide_right.jpg', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>slide_left.jpg', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>admin_left_bar.jpg', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>bg_top_line.jpg', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>admin_right_bar.jpg', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>icon_inactive.gif', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>icon_active.gif', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>icon_edit.gif', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>icon_add.gif', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>icon_delete.gif', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>delete_image.png', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>icon_open.gif', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>icon_password.gif', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>loader.gif', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>icon_error.gif', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>icon_changeaccess.gif', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>icon_lock.gif', 
					'<?php echo CLOUD_STATIC_ADMIN; ?>icon_unlock.gif');
<?php if($phpPage == "admin_config_manager.php"){?>
	colorPicker();
<?php } ?>
}
	...
	oHTTP.onreadystatechange=function() {
		if (oHTTP.readyState==4) {
			...
			if (getValue=="none") {
				...
				document.getElementById('menuDisp').src = "<?php echo CLOUD_STATIC_ADMIN; ?>slide_right.jpg";
			}
			else{
				document.getElementById('menuPortion').style.display = "";
				document.getElementById('menuDisp').src = "<?php echo CLOUD_STATIC_ADMIN; ?>slide_left.jpg";
			}
		}
	}
	oHTTP.send(null);
...
function deleteConfirmRecord(path, toDelete,id){
...
function changeStatus_M(tname, idname, changeMode, ID, statusPortion){
	if(changeMode!="" && ID!="" && statusPortion!=""){
		document.getElementById(statusPortion).innerHTML = '<img src="<?php echo CLOUD_STATIC_ADMIN; ?>loader.gif" align="absmiddle" alt="Loader" border="0" title="Please Wait" class="changeStatus" />';
...
function changeStatusResponse_M(page, changeMode, ID, statusPortion, tname, idname) {
	...
	oHTTP.onreadystatechange=function() {
		if (oHTTP.readyState==4) {
			var getValue=oHTTP.responseText;			
			if (getValue=="none") {
				document.getElementById(statusPortion).innerHTML = '<img src="<?php echo CLOUD_STATIC_ADMIN; ?>icon_inactive.gif" align="absmiddle" alt="Inactive" border="0" onclick="javascript: changeStatus_M(\''+tname+'\',\''+idname+'\',\''+changeMode+'\', '+ID+', \''+statusPortion+'\');" title="Change Status" class="changeStatus" />';
			}
			else{
				document.getElementById(statusPortion).innerHTML = '<img src="<?php echo CLOUD_STATIC_ADMIN; ?>icon_active.gif" align="absmiddle" alt="Active" border="0" onclick="javascript: changeStatus_M(\''+tname+'\',\''+idname+'\',\''+changeMode+'\', '+ID+', \''+statusPortion+'\');" title="Change Status" class="changeStatus" />';
			}
		}
	}
	oHTTP.send(null);
...
function changeStatusResponse(page, changeMode, ID, statusPortion) {
	...
	oHTTP.onreadystatechange=function() {
		if (oHTTP.readyState==4) {
			var getValue=oHTTP.responseText;
			if (getValue=="none") {
				document.getElementById(statusPortion).innerHTML = '<img src="<?php echo CLOUD_STATIC_ADMIN; ?>icon_inactive.gif" align="absmiddle" alt="Inactive" border="0" onclick="javascript: changeStatus(\''+changeMode+'\', '+ID+', \''+statusPortion+'\');" title="Change Status" class="changeStatus" />';
			}
			else{
				document.getElementById(statusPortion).innerHTML = '<img src="<?php echo CLOUD_STATIC_ADMIN; ?>icon_active.gif" align="absmiddle" alt="Active" border="0" onclick="javascript: changeStatus(\''+changeMode+'\', '+ID+', \''+statusPortion+'\');" title="Change Status" class="changeStatus" />';
			}
		}
	}
	oHTTP.send(null);
...
function changeSSLStatusResponse(page, changeMode, ID, statusPortion) {
	...
	oHTTP.onreadystatechange=function() {
		if (oHTTP.readyState==4) {
			var getValue=oHTTP.responseText;
			if (getValue=="none") {
				document.getElementById(statusPortion).innerHTML = '<img src="<?php echo CLOUD_STATIC_ADMIN; ?>icon_unlock.gif" align="absmiddle" alt="Inactive" border="0" onclick="javascript: changeSSLStatus(\''+changeMode+'\', '+ID+', \''+statusPortion+'\');" title="Change Status" class="changeStatus" />';
			}
			else{
				document.getElementById(statusPortion).innerHTML = '<img src="<?php echo CLOUD_STATIC_ADMIN; ?>icon_lock.gif" align="absmiddle" alt="Active" border="0" onclick="javascript: changeSSLStatus(\''+changeMode+'\', '+ID+', \''+statusPortion+'\');" title="Change Status" class="changeStatus" />';
			}
		}
	}
	oHTTP.send(null);
...
function deleteImage(changeMode, ID, statusPortion, type){
	if(confirm('Are you sure to delete this '+type+'?')){
		if(changeMode!="" && ID!="" && statusPortion!=""){
			document.getElementById(statusPortion).innerHTML = '<img src="<?php echo CLOUD_STATIC_ADMIN; ?>loader.gif" align="absmiddle" alt="Loader" border="0" title="Please Wait" class="changeStatus" /> Please wait';
...
function deletePhoto(divID, photo, type,key){
	...
	$.post(url, {mode: 'delete_poster', poster_image: photo, type: type,key: key, folder: $("#random").val(), poster_id: $('#poster_id').val()}, function(retunedData, textStatus){
        if(textStatus == 'success' && retunedData == 'invalid_request'){
        	$("#err"+divID).html('<div class="err">Last image cannot be deleted.</div>');
...
function approveAuction(id, is_approved, searchDisp, auction_type)
{
 if(confirm("Are you sure to perform the action!")){
	...
	$("#td_"+id).html('<img src="<?php echo CLOUD_STATIC_ADMIN; ?>loader.gif" align="absmiddle" alt="Loader" border="0" title="Please Wait" class="changeStatus" />');
			...
			/*if(searchDisp != '' || searchDisp != 'pending'){
				...
				if(is_approved == 1){
                	if(auction_type == 'monthly'){
						$("#td_"+id).html('<img src="<?php echo CLOUD_STATIC_ADMIN; ?>icon_active.gif" align="absmiddle" alt="Mark as Received" border="0" onclick="javascript: approveReceived('+id+', \''+searchDisp+'\');" title="Mark as Received" class="changeStatus" />');
                    }else{
