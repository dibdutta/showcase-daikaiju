...
function postBid(auction_id, user_id,buy_now)
{
	...
	if($('#bid_price_'+auction_id).val() == ''){
    	...
    	alert("Please enter bid amount more than next bid amount $"+curr_bid+".00");
        $('#bid_price_'+auction_id).val("");
        return;
    }
 
	if(confirm("Please confirm your bid of $"+bid_price+".00 \n (Bid may not contain decimals)")){
      
      if(buy_now > 1){
      var bid_price=Math.floor($('#bid_price_'+auction_id).val());
      if(bid_price < buy_now){  
        var url = "mybuying.php";   
        $.get(url, {mode : 'post_bid', auction_id : auction_id,curr_bid:curr_bid,next_increment:next_increment, bid_amount : bid_price}, function(data, textStatus){
            alert(data);
        });
       }else{
       	...
       	var url = "mybuying.php";   
        $.post(url, {mode : 'post_bid', auction_id : auction_id,curr_bid:curr_bid,next_increment:next_increment, bid_amount : bid_price}, function(data, textStatus){
            alert(data);
        });
       }
    }
...
{
    if(confirm("Please confirm your offer(s) or bid(s)")){   
                        ...
                        if(parseInt(bidDataArr[i]['bid_count']) > 0){
                            ...
                            if(curr_user!=''){
                               if(curr_user==highest_user){
                                 var winner= "<img src='http://c4808190.r90.cf2.rackcdn.com/winning-bid-img.png' alt='Winner' title='Winner'>";
                               }else{
                                 //var winner= "<img src='http://c4808190.r90.cf2.rackcdn.com/losing-bid-img.png' alt='Loosing' title='Loosing'>";
                               }
                            }
            ...
            for(var i=0; i<bidDataArr.length; i++){
                if(parseInt(bidDataArr[i]['auction_is_sold']) != 0 && parseInt(bidDataArr[i]['auction_is_sold']) != 3 ){
                    ...
                    if(parseInt(bidDataArr[i]['offer_count']) > 0){
                        ...
                        //alert(bidDataArr[i]['next_increment']);
                        var nextoffer = parseFloat(bidDataArr[i]['last_offer_amount'])+ parseFloat(bidDataArr[i]['next_increment']);
                        dispData = '<div class="auction-row" style=" position:relative;"><div class="buy-text bold offer_buyprice" style="margin:0px 7px 0;"><span class="CurrentBidOffer"  style="font-size:12px; color:#000; ">Current Offer:</span></div><div class="buy-text offer_buyprice" style="font-size:13px; margin-top:1px;"><input type="hidden" id="current_bid_'+bidDataArr[i]['auction_id']+'" value='+bidDataArr[i]['last_offer_amount']+'>$'+bidDataArr[i]['last_offer_amount']+'</div><div class="buy-text-detpstr" style="cursor:pointer; position:relative;" onMouseOver="toggleDiv('+bidDataArr[i]['auction_id']+',1,1,1)" onMouseOut="toggleDiv('+bidDataArr[i]['auction_id']+',0,1,0)"  ><b class="OfferBidNumber"  style="font-size:13px; margin:-7px 0 0 0; float:left; ">'+bidDataArr[i]['offer_count']+' Offer(s)</b></div></div>';
                        $('#auction_data_'+bidDataArr[i]['auction_id']).html(dispData);
                        $('#auction_data_'+bidDataArr[i]['auction_id']).show();
                        /* Offer data onds */
                    }else if(parseInt(bidDataArr[i]['offer_count']) < 1){
                        ...
                        if(parseInt(bidDataArr[i]['bid_count']) > 0){
                            ...
                            dispData = '<div class="auction-row" style="padding:0px;"><div class="buy-text bold"><span class="CurrentBidOffer"  style="font-size:12px; color:#000;">Current:</span> </div><div class="buy-text offer_buyprice" style="font-size:15px;"><input type="hidden" id="is_sold_track_'+bidDataArr[i]['auction_id']+'" value="0"><input type="hidden" id="current_bid_'+bidDataArr[i]['auction_id']+'" value='+nextbid+' >$'+bidDataArr[i]['last_bid_amount']+'</div><div style="cursor:pointer;" class="buy-text-detpstr" onMouseOver="toggleDiv('+bidDataArr[i]['auction_id']+',1,0,1)" onMouseOut="toggleDiv('+bidDataArr[i]['auction_id']+',0,0,0)" >&nbsp;<b class="OfferBidNumber" style="font-size:15px;">'+bidDataArr[i]['bid_count']+'Bid(s)&nbsp;&nbsp;'+winner+'</b></div><div class="buy-text-detpstr bold"><input type="hidden" id="auction_increment_'+bidDataArr[i]['auction_id']+'" value='+bidDataArr[i]['next_increment']+'><span class="nextbid">Next minimum needs to be $'+nextbid+'.00</span></div></div>';
                            $('#auction_data_'+bidDataArr[i]['auction_id']).html(dispData);
                            $('#auction_data_'+bidDataArr[i]['auction_id']).show();
                        }else{
                           dispData = '<div class="text bold"><input type="hidden" id="is_sold_track_'+bidDataArr[i]['auction_id']+'" value="0"></div>';
...
function timeLeftPosterDetails(dataArr)
{
    if(dataArr != null){
        ...
        var curr_user = '<?php echo $_SESSION['sessUserID'];?>';
        $.post(url, {mode : 'time_left', ids : ids}, function(data, textStatus){
            bidDataArr = eval(data);
            for(var i=0; i<bidDataArr.length; i++){
                if(parseInt(bidDataArr[i]['auction_is_sold']) != 0 && parseInt(bidDataArr[i]['auction_is_sold']) != 3){
                    if(parseInt(bidDataArr[i]['fk_auction_type_id']) != 1 && parseInt(bidDataArr[i]['fk_auction_type_id']) != 4){
                    ...
                    if(parseInt(bidDataArr[i]['offer_count']) > 0){
                        ...
                        //alert(bidDataArr[i]['next_increment']);
                        var nextoffer = parseFloat(bidDataArr[i]['last_offer_amount'])+ parseFloat(bidDataArr[i]['next_increment']);
                        dispData = '<div class="auction-row" style="position:relative; cursor:pointer;"><div class="buy-text bold"><span class="CurrentBidOffer" style="font-size:12px; color:#000;">Current Offer:</span></div><div class="buy-text offer_buyprice"><input type="hidden" id="current_bid_'+bidDataArr[i]['auction_id']+'" value='+bidDataArr[i]['last_offer_amount']+'>$'+bidDataArr[i]['last_offer_amount']+'</div><div class="buy-text-detpstr" onMouseOver="toggleDiv('+bidDataArr[i]['auction_id']+',1,1,1)" onMouseOut="toggleDiv('+bidDataArr[i]['auction_id']+',0,1,0)" ><b class="OfferBidNumber" style="margin:-10px 0 0 0;">'+bidDataArr[i]['offer_count']+' Offer(s)</b></div></div>';
                        $('#auction_data_'+bidDataArr[i]['auction_id']).html(dispData);
                        $('#auction_data_'+bidDataArr[i]['auction_id']).show();
                        /* Offer data onds */
                    }else if(parseInt(bidDataArr[i]['offer_count']) < 1){
   ...
   if(data=='1'){
    ...
    if(flag == 'buyer'){
        $.post(url, {mode : 'accept_offer', type : 'buyer', auction_id : auction_id, offer_id : cntr_offer_id}, function(data, textStatus){
            if(data == 'true'){
            alert('You have accepted the counter offer.\rAfter Admin approves you will receive an invoice under Invoice menu for payment.');
        ...
        $.post(url, {mode : 'accept_offer', type : 'seller', auction_id : auction_id, offer_id : offer_id}, function(data, textStatus){
            if(data == 'true'){
            alert('You have accepted the offer.\rAfter Admin approves, buyer will receive an invoice for payment.');
