...
if(! isset($token)) {
	...
	$personName        = $_SESSION['shipping_info']['shipping_firstname']." ".$_SESSION['shipping_info']['shipping_lastname'];
	$SHIPTOSTREET      = $_SESSION['shipping_info']['shipping_address1'].",".$_SESSION['shipping_info']['shipping_address2'];
	$SHIPTOCITY        = $_SESSION['shipping_info']['shipping_city'];
	$SHIPTOSTATE	      = $_SESSION['shipping_info']['shipping_state'];
	$SHIPTOCOUNTRYCODE = $_SESSION['shipping_info']['shipping_country_code'];
	$SHIPTOZIP         = $_SESSION['shipping_info']['shipping_zipcode'];	
   //	$L_NAME0           = "TEST";
   //	$L_AMT0            = 2;
   //	$L_QTY0            = 1;
   //	$L_NAME1           = "TEST";
	...
	*/
   	$returnURL =urlencode(SITE_URL.'/classes/paypal_pro_express/ReviewOrder.php?currencyCodeType='.$currencyCodeType.'&paymentType='.$paymentType);
   	$cancelURL =urlencode(SITE_URL."cart.php?mode=do_express_checkout" );
	...
	*/
	...
	foreach($_SESSION['cart'] as $key => $value){
   	...
   	$itemamt = number_format($cartSubTotal, 2, ".", "");
	$amt =  number_format($cartSubTotal + $_SESSION['shipping_info']['shipping_charge'] + $_SESSION['shipping_info']['sale_tax_amount'], 2, ".", "");
    ...
    */
	...
	$shiptoAddress = "&SHIPTONAME=$personName&SHIPTOSTREET=$SHIPTOSTREET&SHIPTOCITY=$SHIPTOCITY&SHIPTOSTATE=$SHIPTOSTATE&SHIPTOCOUNTRYCODE=$SHIPTOCOUNTRYCODE&SHIPTOZIP=$SHIPTOZIP";
	$nvpstr="&ADDRESSOVERRIDE=1".$shiptoAddress."&".$itemInfo."AMT=".$amt."&ITEMAMT=".$itemamt."&SHIPPINGAMT=".number_format($_SESSION['shipping_info']['shipping_charge'], 2, ".", "")."&TAXAMT=".number_format($_SESSION['shipping_info']['sale_tax_amount'], 2, ".", "")."&ReturnUrl=".$returnURL."&CANCELURL=".$cancelURL ."&CURRENCYCODE=".$currencyCodeType."&PAYMENTACTION=".$paymentType;
	$nvpstr = $nvpHeader.$nvpstr;
   ...
   if($ack == 'SUCCESS' || $ack == 'SUCCESSWITHWARNING'){
		...
		echo $_SESSION['token']=$_REQUEST['token'];
		echo "<br>";
		echo $_SESSION['payer_id'] = $_REQUEST['PayerID'];
		echo "<br>";
		echo $_SESSION['paymentAmount']=$_REQUEST['paymentAmount'];
		echo "<br>";
		echo $_SESSION['currCodeType']=$_REQUEST['currencyCodeType'];
		echo "<br>";
		echo "paymentType==".$_SESSION['paymentType']=$_REQUEST['paymentType'];
   	...
   	//exit;
   		header("location:".SITE_URL."cart.php?mode=do_express_checkout_details&token=".$_REQUEST['token']);
	    exit;
   } else  {
		//Redirecting to APIError.php to display errors.
		//$location = "APIError.php";
