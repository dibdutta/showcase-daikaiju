...
if(! isset($token)) {
	...
	*/
   	...
   	$itemamt = number_format($invoiceData[0]['total_amount']);
    ...
    */
	...
	$shiptoAddress = "&SHIPTONAME=$personName&SHIPTOSTREET=$SHIPTOSTREET&SHIPTOCITY=$SHIPTOCITY&SHIPTOSTATE=$SHIPTOSTATE&SHIPTOCOUNTRYCODE=$SHIPTOCOUNTRYCODE&SHIPTOZIP=$SHIPTOZIP";
	$nvpstr="&ADDRESSOVERRIDE=1".$shiptoAddress."&".$itemInfo."AMT=".$amt."&ITEMAMT=".$itemamt."&SHIPPINGAMT=".number_format($_SESSION['invoice_'.$_REQUEST['invoice_id']]['shipping_info']['shipping_charge'], 2, ".", "")."&TAXAMT=".number_format($_SESSION['invoice_'.$_REQUEST['invoice_id']]['shipping_info']['sale_tax_amount'], 2, ".", "")."&ReturnUrl=".$returnURL."&CANCELURL=".$cancelURL ."&CURRENCYCODE=USD&PAYMENTACTION=Sale";
	$nvpstr = $nvpHeader.$nvpstr;
   ...
   if($ack == 'SUCCESS' || $ack == 'SUCCESSWITHWARNING'){
		...
		$_SESSION['invoice_'.$_SESSION['invoice_id']]['TotalAmount']= $resArray['AMT'] + $resArray['SHIPDISCAMT'];
