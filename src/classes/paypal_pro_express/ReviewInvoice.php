...
if(!isset($token)) {
	...
	foreach($invoiceData[0]['discounts'] as $key => $value){
        $discount += $value['amount'];
	}
	foreach($invoiceData[0]['additional_charges'] as $key => $value){
        $charges += $value['amount'];
	}
	
	/*echo "<pre>";
	print_r($_SESSION['invoice_'.$_REQUEST['invoice_id']]);
	echo "</pre>";
	die();*/
	...
	$personName        = $_SESSION['invoice_'.$_REQUEST['invoice_id']]['shipping_info']['shipping_firstname']." ".$_SESSION['invoice_'.$_REQUEST['invoice_id']]['shipping_info']['shipping_lastname'];
	$SHIPTOSTREET      = $_SESSION['invoice_'.$_REQUEST['invoice_id']]['shipping_info']['shipping_address1'].",".$_SESSION['shipping_info']['shipping_address2'];
	$SHIPTOCITY        = $_SESSION['invoice_'.$_REQUEST['invoice_id']]['shipping_info']['shipping_city'];
	$SHIPTOSTATE	      = $_SESSION['invoice_'.$_REQUEST['invoice_id']]['shipping_info']['shipping_state'];
	$SHIPTOCOUNTRYCODE = $_SESSION['invoice_'.$_REQUEST['invoice_id']]['shipping_info']['shipping_country_code'];
	$SHIPTOZIP         = $_SESSION['invoice_'.$_REQUEST['invoice_id']]['shipping_info']['shipping_zipcode'];	
   //	$L_NAME0           = "TEST";
   //	$L_AMT0            = 2;
   //	$L_QTY0            = 1;
   //	$L_NAME1           = "TEST";
	...
	*/
   	$returnURL =urlencode('https://'.$_SERVER['HTTP_HOST'].'/classes/paypal_pro_express/ReviewInvoice.php?currencyCodeType='.$currencyCodeType.'&paymentType='.$paymentType);
   	$cancelURL =urlencode("https://".$_SERVER['HTTP_HOST']."/my_invoice.php");

	/* Construct the parameter string that describes the PayPal payment
		the varialbes were set in the web form, and the resulting string
		is stored in $nvpstr
	*/
    $shipping_charges= number_format(($_SESSION['invoice_'.$_REQUEST['invoice_id']]['shipping_info']['shipping_charge'] + $charges), 2, ".", "");
   	$itemamt = number_format(($invoiceData[0]['total_amount']+ $discount - $charges), 2, ".", "");
	$amt =  number_format(($invoiceData[0]['total_amount'] + $_SESSION['invoice_'.$_REQUEST['invoice_id']]['shipping_info']['shipping_charge'] + $_SESSION['invoice_'.$_REQUEST['invoice_id']]['shipping_info']['sale_tax_amount']), 2, ".", "");
 	/* 
   ...
   $_SESSION['invoice_'.$_SESSION['invoice_id']]['reshash']=$resArray;
   $ack = strtoupper($resArray["ACK"]);
   ...
   if($ack == 'SUCCESS' || $ack == 'SUCCESSWITHWARNING'){
		...
		echo $_SESSION['invoice_'.$_SESSION['invoice_id']]['token']=$_REQUEST['token'];
		echo "<br>";
		echo $_SESSION['invoice_'.$_SESSION['invoice_id']]['payer_id'] = $_REQUEST['PayerID'];
		echo "<br>";
		echo $_SESSION['invoice_'.$_SESSION['invoice_id']]['paymentAmount']=$_REQUEST['paymentAmount'];
		echo "<br>";
		echo $_SESSION['invoice_'.$_SESSION['invoice_id']]['currCodeType']=$_REQUEST['currencyCodeType'];
		echo "<br>";
		echo "paymentType==".$_SESSION['invoice_'.$_SESSION['invoice_id']]['paymentType']=$_REQUEST['paymentType'];
		
		
		$resArray=$_SESSION['invoice_'.$_SESSION['invoice_id']]['reshash'];
		$_SESSION['invoice_'.$_SESSION['invoice_id']]['TotalAmount']= $resArray['AMT'];
   	
   	//exit;
   		header("location:https://".$_SERVER['HTTP_HOST']."/my_invoice.php?mode=do_express_checkout_details&invoice_id=".$_SESSION['invoice_id']."&token=".$_REQUEST['token']);
	    exit;
   } else  {
		//Redirecting to APIError.php to display errors.
		//$location = "APIError.php";
		...
		$_SESSION['Err'] = "Payment failed. Please try again!"."<br/>"." Paypal Error Code:". $resArray['L_ERRORCODE0']."&nbsp;".$resArray['L_LONGMESSAGE0'];
		header("location:https://".$_SERVER['HTTP_HOST']."/my_invoice.php?mode=do_express_checkout&invoice_id=".$_SESSION['invoice_id']);
		exit;
	  }
}
