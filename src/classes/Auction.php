...
class Auction extends DBCommon{
	...
	*/
	...
	function countLiveAuctions($fetch = '',$auction_week_id = '')
	{
		...
		if(($fetch != 'weekly') and ($fetch != 'extended')){
			$sql = "SELECT count(a.auction_id) AS counter
				FROM tbl_auction a 				
	...
	function searchPosterIds($list='',$auction_week_id='')
	{
        ...
        if(isset($_REQUEST['shop_cat_id']) && $_REQUEST['shop_cat_id'] != '' && $_REQUEST['keyword'] == ''){
            if($list=='weekly'){
                $sql = "SELECT DISTINCT ptsc.fk_poster_id FROM tbl_poster_to_shop_category_live ptsc, tbl_poster_images_live pi, tbl_auction_live a
                        WHERE ptsc.fk_poster_id = a.fk_poster_id
                        AND pi.fk_poster_id = a.fk_poster_id
                        AND pi.is_default='1'
                        AND ptsc.fk_shop_cat_id = '".(int)$_REQUEST['shop_cat_id']."'";
                $sql .= $qry;
	...
	function select_details_auction($auction_id,$is_fixed='',$is_extended='')
	{
		...
		if($is_fixed==1){
			...
			if($count<1){
				$auction_id=$this->getAfterSoldAuctionID($auction_id);
			}
		}else{
			$count=$this->isLiveAuctionItem($auction_id);
		}
	...
	function bidWinnerForReport($auction_id){
		...
		$res=mysqli_fetch_assoc($res_sql);
	...
	{
		$user_id = isset($_SESSION['sessUserID']) ? $_SESSION['sessUserID'] : '';
		$dataArr = [];
		$to = 12; // default limit

		// Determine table names based on fetch type
		if ($fetch === 'weekly') {
			$auctionTable = "tbl_auction_live";
			$posterTable = "tbl_poster_live";
			$posterImageTable = "tbl_poster_images_live";
		} else {
			$auctionTable = TBL_AUCTION;
			$posterTable = TBL_POSTER;
			$posterImageTable = TBL_POSTER_IMAGES;
		}

		// Handle "first poster info" for fixed and weekly
		$firstPoster = null;
		if ($fetch === 'fixed' || $fetch === 'weekly') {
			$typeId = $fetch === 'fixed' ? 1 : 2;

			$firstSql = "SELECT 
							MAX(tw.watching_id) AS watch_indicator,
							a.auction_id,
							a.fk_auction_type_id,
							a.auction_asked_price,
							a.auction_buynow_price,
							p.poster_id,
							p.poster_title,
							MAX(pi.poster_thumb) AS poster_thumb,
							MAX(pi.is_cloud) AS is_cloud,
							a.max_bid_amount AS last_bid_amount
						FROM {$auctionTable} a
						INNER JOIN {$posterTable} p ON a.fk_poster_id = p.poster_id
						INNER JOIN {$posterImageTable} pi ON a.fk_poster_id = pi.fk_poster_id
						LEFT JOIN " . TBL_WATCHING . " tw ON a.auction_id = tw.auction_id AND tw.user_id = '{$user_id}'
						WHERE a.slider_first_position_status = '1'
						AND pi.is_default = '1'
						AND a.fk_auction_type_id = '{$typeId}'
						AND a.auction_is_approved = '1'
						AND a.auction_is_sold = '0'";

			if ($fetch === 'weekly') {
				$firstSql .= " AND a.auction_actual_start_datetime <= NOW() 
							AND a.auction_actual_end_datetime >= NOW()";
			}

			$firstSql .= " GROUP BY a.auction_id
						ORDER BY a.auction_id ASC
						LIMIT 1";

			$firstRs = mysqli_query($GLOBALS['db_connect'], $firstSql);
			if ($firstRs && $firstRow = mysqli_fetch_assoc($firstRs)) {
				if (!empty($firstRow['poster_thumb'])) {
					$dataArr[] = $firstRow;
				} else {
					// if no valid poster_thumb, increase the limit for main query
					$to = 12;
				}
			}
		}

		// Base SQL for remaining auctions
		$sql = "SELECT 
					MAX(tw.watching_id) AS watch_indicator,
					a.auction_id,
					a.fk_auction_type_id,
		...
		// Conditions by fetch type
		if ($fetch === 'fixed') {
			$sql .= " AND a.fk_auction_type_id = '1' AND a.auction_is_sold = '0' AND a.auction_is_approved = '1'
					AND a.slider_first_position_status = '0'";
		} elseif ($fetch === 'weekly') {
			$sql .= " AND a.fk_auction_type_id = '2' AND a.auction_is_sold = '0' AND a.auction_is_approved = '1'
					AND a.slider_first_position_status = '0'
					AND a.auction_actual_start_datetime <= NOW()
					AND a.auction_actual_end_datetime >= NOW()";
		} elseif ($fetch === 'monthly') {
			$sql .= " AND a.fk_auction_type_id = '3' AND a.auction_is_approved = '1' AND a.is_approved_for_monthly_auction = '1'
					AND a.auction_is_sold = '0'
					AND a.auction_actual_start_datetime <= NOW()
					AND a.auction_actual_end_datetime >= NOW()";
			$to = 6;
		...
		// GROUP BY to satisfy MySQL 8 ONLY_FULL_GROUP_BY
...
function soldAuctionMONTHLY($auctionStatus = '', $user_id = '',$sort_type='',$search_fixed_poster='',$start_date='',$end_date='')
   ...
   function fetchEditAuctionDetail($auction_id,$chkLive='')
   {
	...
	function sellerInvoicePrice($auction_id){
		...
		if($rs = mysqli_fetch_assoc(mysqli_query($GLOBALS['db_connect'],$sql))){
			return $rs['amount'];
		}
	}
	...
	function fetchExperiedAuctions($fetch = '', $poster_ids = '', $view_mode = '', $auction_week_id = '')
	{
		...
		if ($view_mode === 'list') {
			...
			SELECT
				...
				MAX(a.is_reopened)                     AS is_reopened,
				MAX(a.fk_auction_type_id)              AS fk_auction_type_id,
				MAX(a.auction_asked_price)             AS auction_asked_price,
				MAX(a.fk_auction_week_id)              AS fk_auction_week_id,
				MAX(a.auction_buynow_price)             AS auction_buynow_price,
				MAX(a.auction_actual_start_datetime)   AS auction_actual_start_datetime,
				MAX(a.auction_actual_end_datetime)     AS auction_actual_end_datetime,
				MAX(a.auction_is_sold)                 AS auction_is_sold,

				MAX(c.cat_value)   AS poster_size,
				MAX(c1.cat_value)  AS genre,
				MAX(c2.cat_value)  AS decade,

				poster_size(p.poster_id) AS country,
				cond(p.poster_id)        AS cond,

				MAX(aw.auction_week_title) AS auction_week_title,

				p.poster_id,
				MAX(p.fk_user_id)   AS fk_user_id,
				MAX(p.poster_title) AS poster_title,

				MAX(pi.poster_thumb) AS poster_thumb,
				MAX(pi.is_cloud)     AS is_cloud,

				MAX(a.max_bid_amount) AS last_bid_amount,
				MAX(a.bid_count)      AS bid_count

			FROM " . TBL_AUCTION . " a
			INNER JOIN " . TBL_POSTER . " p 
				ON a.fk_poster_id = p.poster_id
			INNER JOIN " . TBL_POSTER_IMAGES . " pi
				ON a.fk_poster_id = pi.fk_poster_id AND pi.is_default = '1'
			LEFT JOIN " . TBL_AUCTION_WEEK . " aw
				ON a.fk_auction_week_id = aw.auction_week_id
			LEFT JOIN tbl_poster_to_category ptc
				ON ptc.fk_poster_id = p.poster_id
		...
		=========================== */
			...
			SELECT
				...
				MAX(a.fk_auction_type_id) AS fk_auction_type_id,
				MAX(a.auction_week_id)    AS auction_week_id,
				MAX(aw.auction_week_title) AS auction_week_title,

				MAX(p.poster_id)    AS poster_id,
				MAX(p.poster_title) AS poster_title,
				MAX(p.fk_user_id)   AS fk_user_id,

				MAX(a.poster_thumb) AS poster_thumb,
		...
		elseif ($fetch === 'monthly') {
			...
			$sql .= "
			AND (
				(a.fk_auction_type_id = '1')
				OR (a.fk_auction_type_id = '2'
					AND a.auction_actual_start_datetime <= NOW()
					AND a.auction_actual_end_datetime >= NOW())
				OR (a.fk_auction_type_id = '3'
					AND a.is_approved_for_monthly_auction = '1'
					AND a.auction_actual_start_datetime <= NOW()
					AND a.auction_actual_end_datetime >= NOW())
			)
		...
		$sql .= " GROUP BY a.auction_id, p.poster_id ";

		if ($this->orderBy === "auction_actual_end_datetime") {
			$sql .= " ORDER BY auction_actual_end_datetime {$this->orderType} ";
		}
		elseif ($this->orderBy === "auction_bid_price") {
			$sql .= " ORDER BY last_bid_amount {$this->orderType} ";
		}
		elseif (isset($_REQUEST['order_by']) && $_REQUEST['order_by'] === "poster_title") {
			$sql .= " ORDER BY poster_title {$_REQUEST['order_type']} ";
		}
		else {
			$sql .= " ORDER BY last_bid_amount {$this->orderType} ";
		}
		...
		// echo $sql; // enable only for debugging
	...
	function countLiveWeeklyAuctions(){
		...
		if($resSql){
			$fetchSql = mysqli_fetch_assoc($resSql);
	...
	{
		$watchUserId = isset($_SESSION['sessUserID']) ? $_SESSION['sessUserID'] : '';
		$dataArr = [];
		$to = 11; // default limit
		...
		$posterTable = "tbl_poster_live";
		$posterImageTable = "tbl_poster_images_live";
		$auctionWeekTable = TBL_AUCTION_WEEK;
		$watchingTable = TBL_WATCHING;

		// --- First Poster Info for slider ---
		$firstPosterSql = "SELECT 
								...
								MAX(pi.poster_thumb) AS poster_thumb
							FROM {$auctionTable} a
							LEFT JOIN {$posterTable} p ON a.fk_poster_id = p.poster_id
							LEFT JOIN {$posterImageTable} pi ON a.fk_poster_id = pi.fk_poster_id
							LEFT JOIN {$auctionWeekTable} aw ON a.fk_auction_week_id = aw.auction_week_id
							LEFT JOIN {$watchingTable} tw ON a.auction_id = tw.auction_id AND tw.user_id = '{$watchUserId}'
							WHERE a.slider_first_position_status = '1'
							AND a.auction_actual_start_datetime > NOW()
							AND pi.is_default = '1'
							AND a.fk_auction_type_id = '2'
							...
							AND a.auction_is_sold = '0'
							AND aw.is_test = '0'
							GROUP BY a.auction_id, a.auction_asked_price, p.poster_id, p.poster_title
							ORDER BY a.auction_id ASC
							LIMIT 1";

		$firstRs = mysqli_query($GLOBALS['db_connect'], $firstPosterSql);
		if ($firstRs && $firstRow = mysqli_fetch_assoc($firstRs)) {
			if (!empty($firstRow['poster_thumb'])) {
				$dataArr[] = $firstRow;
			} else {
				$to = 12; // Increase limit if no valid first poster
			}
		}

		// --- Remaining upcoming auctions ---
		$sql = "SELECT 
					MAX(tw.watching_id) AS watch_indicator,
					a.auction_id,
					a.auction_asked_price,
...
function fetchStillsLiveAuctions($view_mode=''){
	...
	function OrderBySoldPrice($sort_by = '', $sort_type = '',$title='',$item_type='',$auction_week_id=''){
	 ...
	 function homePageSoldSlider() 
	 {
		...
		if ($firstRs && $firstRow = mysqli_fetch_assoc($firstRs)) {
			$dataArr[] = $firstRow;
			$limit = 11; // reduce remaining limit
		}

		// --- Remaining sold auctions ---
		// Count total eligible rows for random offset
	...
	function getAfterSoldAuctionID($auction_id){
		$sql = "SELECT auction_id_new
				FROM tbl_auction_mapping
				WHERE  auction_id_old= ".$auction_id;
		$rs = mysqli_query($GLOBALS['db_connect'],$sql);
		while($row = mysqli_fetch_assoc($rs)){
			$auction_id_new = $row["auction_id_new"];
		}
		return $auction_id_new;
	}
}
